...
def write_output(output_ids, values):
    ...
    if len(values) > 0:
        for k in range(0, len(values[0])):
            ...
            for _ in output_ids:
                j__ = values[i][k]
                if isinstance(j__, types.ListType):
                    for jv_ in j__:
                        row.append(jv_)
                else:
                    row.append(j__)
...
def is_string_type(j):
    return isinstance(j, types.StringType) \
           or isinstance(j, types.UnicodeType)
...
def find_and_replace_references(root, j):
    ...
    if isinstance(j, types.ListType) and len(j) > 0:
    ...
    elif isinstance(j, types.DictType):
        obj = {}
...
def add_climate_data_to_env(env, simj, climate_csv_string=""):
    ...
    options["start-date"] = simj["start-date"]
    options["end-date"] = simj["end-date"]
