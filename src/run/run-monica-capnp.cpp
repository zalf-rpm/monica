...
kj::Promise<J11Array> fromCapnpSoilProfile(mas::schema::soil::Profile::Client profile) {
...
kj::Promise<void> RunMonica::run(RunContext context) //override
{
  ...
  auto runMonica = [context, envR, this](DataAccessor da = DataAccessor()) mutable -> kj::Promise<Output> {
    std::string err;
    ...
    if (envR.hasSoilProfile()) {
      auto prom = fromCapnpSoilProfile(envR.getSoilProfile()).then([&env](auto &&layers) {
        env.params.siteParameters.merge(J11Object{{"SoilProfileParameters", layers}});
      });
      proms.add(kj::mv(prom));
    } else {
      proms.add(kj::READY_NOW);
    }

    return kj::joinPromises(proms.finish()).then([&da, &env, this]() {
  ...
  if (envR.hasTimeSeries()) {
    ...
    return rangeProm
        ...
        }).then([context, runMonica](const DataAccessor &da) mutable {
      //auto out = runMonica(da);
      return runMonica(da).then([context](auto &&out) mutable {
        auto rs = context.getResults();
    ...
    return runMonica().then([context](auto &&out) mutable {
      auto rs = context.getResults();
      auto res = rs.initResult();
      res.initStructure().setJson();
      res.setValue(out.toString());
