...
json11::Json createMarch31stResultsMessage(const MonicaModel& monica)
{
  ...
  {to_string(sum30cmSMB_CO2EvolutionRate),
monica.get_sum30cmSMB_CO2EvolutionRate()}, {to_string(NH3Volatilised),
monica.getNH3Volatilised()}, {to_string(sum30cmActDenitrificationRate),
monica.getsum30cmActDenitrificationRate()}, {to_string(leachingNAtBoundary),
monica.nLeaching()} };
...
{
  auto jsonMsg = json11::Json::object{
  {to_string(avg10cmMonthlyAvgCorg), avs[avg10cmMonthlyAvgCorg] /
double(date.daysInMonth())}, {to_string(avg30cmMonthlyAvgCorg),
avs[avg30cmMonthlyAvgCorg] / double(date.daysInMonth())},
  {to_string(mean90cmMonthlyAvgWaterContent), monica.mean90cmWaterContent()},
  {to_string(monthlySumGroundWaterRecharge),
avs[monthlySumGroundWaterRecharge]}, {to_string(monthlySumNLeaching),
avs[monthlySumNLeaching]}, {to_string(maxSnowDepth), monica.maxSnowDepth()},
                                          ...
                                          bool isInProcess)
{
  ...
  debug() << "MONICA: connecting monica zeromq input socket to address: " <<
inputSocketAddress << endl; try
  {
    input.connect(inputSocketAddress.c_str());
    debug() << "MONICA: connected monica zeromq input socket to address: " <<
inputSocketAddress << endl;

    zmq::socket_t output_(*zmqContext, ZMQ_PUSH);
    zmq::socket_t& output = isInProcess ? input : output_;
    if(isInProcess)
      debug() << "MONICA: using monica zeromq pair input socket also as output
socket at address: " << outputSocketAddress << endl; else
    {
      debug() << "MONICA: binding monica zeromq output socket to address: " <<
outputSocketAddress << endl; output.bind(outputSocketAddress.c_str()); debug()
<< "MONICA: bound monica zeromq output socket to address: " <<
outputSocketAddress << endl;
    }
...
readUserParameterFromDatabase(initMsg["centralParameterType"].int_value());
          ...
          else if(msgType == "dailyStep")
          {
            if(!monicaUPtr)
            {
              cout << "Error: No initMonica message has been received yet,
dropping message " << msg.toString() << endl; continue;
            }
...
false); map<Climate::ACD, double> climateData = {{Climate::tmin,
dsm["tmin"].number_value()}, {Climate::tavg, dsm["tavg"].number_value()},
                                                     {Climate::tmax,
dsm["tmax"].number_value()}, {Climate::precip, dsm["precip"].number_value()},
                                                     {Climate::wind,
dsm["wind"].number_value()}, {Climate::globrad, dsm["globrad"].number_value()},
                                                     {Climate::relhumid,
dsm["relhumid"].number_value()}};

            debug() << "currentDate: " << date.toString() << endl;

            auto dailyStepResultMsg = Json::object{{"date",
              ...
              for(auto ws : msg.json["worksteps"].array_items())
              {
                ...
                else if(wsType == "Harvest")
                {
                  ...
                  dailyStepResultMsg["harvesting"] =
                    createHarvestingMessage(*cropResult.get(),
*monicaUPtr.get());

                  //to count the applied fertiliser for the next production
process monicamodel::resetFertiliserCounter(&monica); crop.reset(); prevDevStage
= 0;
                }
            ...
            if(date.day() == 31 && date.month() == 3)
              dailyStepResultMsg["march31st"] =
createMarch31stResultsMessage(monica);

            if(date.day() == date.daysInMonth())
              dailyStepResultMsg["monthly"] = createMonthlyResultsMessage(date,
aggregatedValues, monica);

            if(date == Date(31, 12, date.year()))
              dailyStepResultMsg["yearly"] =
createYearlyResultsMessage(aggregatedValues);

            int devStage = monica.currentCropModule ?
monica.currentCropModule->vc_DevelopmentalStage + 1 : 0; if(prevDevStage <
devStage)
            {
            ...
            {
              cerr
                << "Exception on trying to send message on zmq push socket with
address: "
                << outputSocketAddress << "! Error: [" << e.what() << "]" <<
endl;
            }
          }
        }
      ...
      {
        cerr
          << "Exception on trying to receive message on zmq pull socket with
address: "
          << inputSocketAddress << "! Will continue to receive push messages!
Error: [" << e.what() << "]" << endl;
      }
    }
  }
