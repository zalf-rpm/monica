...
Copyright (C) 2007-2013, Leibniz Centre for Agricultural Landscape Research
(ZALF)
...
struct E {
  E()
      : year(2025), station("PRENZLAU"), simulation("star2"), scenario("2K"),
        realization("1"), dontRotateCropRotation(true), heightNN(50),
        slope(0.01), region("uecker"), weisseritzBk50Id(-1),
        ueckerSTR("Al2a03"), farmName("ImpAss"), cropRotationName("WR"),
        useNMinMethod(true), useAutoIrrigation(true), noOfLayers(20),
        layerThickness(0.1), co2(-1), activateDebugOutput(true),
        showGeneralResultsOutput(false), activateOutputFiles(true),
...
void runLandcareDSSMonica(E e) {
  ...
  AvailableClimateData acds[] = {tmin,     tavg, tmax,   globrad,
                                 relhumid, wind, precip, sunhours};

  // one year in front for monica or EOM vorjahr and the 14 years for the
  // statistics
  Date start(1, 1, e.year - 14 - 1);
  // noOfPVs - 1 years for the following PVs in the fruchtfolge + 15 years
  // statistics
  Date end(31, 12, e.year + 15); // + noOfPVs - 1);
  debug() << "start: " << start.toString() << " end: " << end.toString()
          << endl;
  ...
  if (!s) {
    cout << "There is no simulation:" << e.simulation << " supported right now!"
         << endl;
    return;
  }
  ...
  if (!scen) {
    if (s->scenarios().size() == 1) {
      ...
      cout << "There is no scenario: " << e.scenario
           << " but there is only one anyway, so choosing: " << scen->name()
           << " instead of ending." << endl;
    } else {
  ...
  for (ClimateRealization *r : scen->realizations()) {
    if (e.realization == "all" || r->name() == e.realization ||
        scen->realizations().size() == 1)
      das[r->name()] =
          r->dataAccessorFor(vector<ACD>(acds, acds + 8), llc, start, end);
  }
  ...
  if ((env.useNMinMineralFertilisingMethod = e.useNMinMethod)) {
    ...
    env.nMinFertiliserPartition = MineralFertiliserParameters(
        f->name, f->amidn / f->n, f->no3n / f->n, f->no4n / f->n);
  }
  ...
  for (Produktionsverfahren *pv : cr->pvs) {
    ...
    CropPtr crop(new Crop(i.cropId, cps->pc_CropName, shDates[pv].first,
                          shDates[pv].second, cps, rps,
      ...
      if (e.showGeneralResultsOutput) {
        ...
        for (monica::Result::RId2Vector::value_type p : res.generalResults) {
          ...
          avgYearlyGeneralResults[year][rid].insert(
              avgYearlyGeneralResults[year][rid].end(), p.second.begin(),
              p.second.end());
  ...
  for (YearlyCropResults::value_type p : avgYearlyCropResults) {
    ...
    for (CropResults::value_type p2 : p.second) {
      ...
      for (CR::value_type p3 : p2.second) {
        ...
        ResultIdInfo info = resultIdInfo(rid);
        cout << rid << " " << info.name << " [ " << info.unit
             << "] avgValue: " << p4.second << " sigma: " << p4.first << endl;
      }
    }
    cout << "---------------------------" << endl;
  }
  ...
  if (e.showGeneralResultsOutput) {
    cout << "general results:" << endl << "---------------" << endl;
    for (YearlyGeneralResults::value_type p : avgYearlyGeneralResults) {
      ...
      for (GeneralResults::value_type p2 : p.second) {
        ...
        ResultIdInfo info = resultIdInfo(rid);
        cout << rid << " " << info.name << " [ " << info.unit
             << "] avgValue: " << p3.second << " sigma: " << p3.first << endl;
      }
      cout << "---------------------------" << endl;
    }
    cout << "-----------------------------------------" << endl;
  ...
  if (e.showGeneralResultsOutput) {
    ...
    for (GeneralResults::value_type p : avgGeneralResults) {
      ...
      ResultIdInfo info = resultIdInfo(rid);
      cout << rid << " " << info.name << " [ " << info.unit
           << "] avgValue: " << p2.second << " sigma: " << p2.first << endl;
    }
    cout << "-----------------------------------------" << endl;
  }
...
void testLandcareDSS(int argc, char **argv) {
  ...
  if ((argc == 2 &&
       ((argv[1][0] == '-' && argv[1][1] == 'h') || argv[1][0] == 'h')) ||
      (argc > 1 && (argc - 1) % 2 == 1)) {
    cout << "usage: run-monica" << endl
         ...
         << "\t| scenario: " << e.scenario
         << " [A1B | B1] for wettreg or [---] for star" << endl
         << "\t| realization: " << e.realization
         << " [tro_a | nor_a | feu_a | all] for wettreg or [1] for star" << endl
         << "\t| dontRotate: " << (e.dontRotateCropRotation ? "true" : "false")
         << " [true | false]" << endl
         << "\t| heightNN: " << e.heightNN << endl
         << "\t| slope: " << e.slope << endl
         << "\t| region: " << e.region << " [weisseritz | uecker]" << endl
         << "\t| weisseritzBk50Id: " << e.weisseritzBk50Id
         << " (if region: weisseritz)" << endl
         << "\t| ueckerSTR: " << e.ueckerSTR << " (if region: uecker)" << endl
         << "\t| farmName: " << e.farmName << " (start of name is enough)"
         << endl
         << "\t| cropRotationName: " << e.cropRotationName
         << " (start of name is enough)" << endl
         << "\t| useNMinMethod: " << (e.useNMinMethod ? "true" : "false")
         << " [true | false]" << endl
         << "\t| useAutomaticIrrigation: "
         << (e.useAutoIrrigation ? "true" : "false") << " [true | false]"
         << endl
         << "\t| noOfLayers: " << e.noOfLayers << endl
         << "\t| layerThickness: " << e.layerThickness << " in meters" << endl
         << "\t| co2: " << e.co2 << " in ppm"
         << " [-1 means use monica internal CO2-algorithm]" << endl
         << "\t| activateDebugOutput: "
         << (e.activateDebugOutput ? "true" : "false") << " [true | false]"
         << endl
         << "\t| showGeneralResultsOutput: "
         << (e.showGeneralResultsOutput ? "true" : "false") << " [true | false]"
         << endl
         << "\t| activateOutputFiles: "
         << (e.activateOutputFiles ? "true" : "false") << " [true | false]"
         << endl
         << "\t| pathToOutputDir: " << e.pathToOutputDir
         << " [path to dir for output files]" << endl
         << "\t]*" << endl;
    return;
  } else {
    ...
    cout << "running monica with:" << endl
         ...
         << endl
         << "\t| useAutomaticIrrigation: "
         << (e.useAutoIrrigation ? "true" : "false") << endl
         << "\t| noOfLayers: " << e.noOfLayers << endl
         << "\t| layerThickness: " << e.layerThickness << " [m]" << endl
         << "\t| co2: " << e.co2 << endl
         << "\t| activateDebugOutput: "
         << (e.activateDebugOutput ? "true" : "false") << endl
         << "\t| showGeneralResultsOutput: "
         << (e.showGeneralResultsOutput ? "true" : "false") << endl
         << "\t| activateOutputFiles: "
         << (e.activateOutputFiles ? "true" : "false") << endl
...
int main(int argc, char **argv) {
  ...
  // MYSQL* con2 = mysql_real_connect(con, "141.30.160.222", "landcare",
  // "c.1axvPb", "landcare",	3306, NULL, 0); MYSQL* con2 =
  // mysql_real_connect(con, "mysql.zalf.de", "gast", "gast", "eva2",	3306,
  // NULL, 0);

  // string q = "select * from header where stat_id = 1";
  // int success = mysql_real_query(con2, q.c_str(), (unsigned
  // int)strlen(q.c_str()));
  ...
  */
  ...
  CCGermanySimulationConfiguration *config =
      new CCGermanySimulationConfiguration();
  config->setBuekId(23);
