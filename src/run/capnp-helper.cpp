...
ACD monica::climateElementToACD(mas::schema::climate::Element e) {
  switch (e) {
  case mas::schema::climate::Element::TMIN: return ACD::tmin;
  case mas::schema::climate::Element::TAVG: return ACD::tavg;
  case mas::schema::climate::Element::TMAX: return ACD::tmax;
  case mas::schema::climate::Element::PRECIP: return ACD::precip;
  case mas::schema::climate::Element::RELHUMID: return ACD::relhumid;
  case mas::schema::climate::Element::WIND: return ACD::wind;
  case mas::schema::climate::Element::GLOBRAD: return ACD::globrad;
  default: return ACD::skip;
  }
}
...
DataAccessor monica::fromCapnpData(
  ...
  for (capnp::uint i = 0; i < header.size(); i++) {
    ...
    switch (header[i]) {
    case E::TMIN: da.addClimateData(ACD::tmin, kj::mv(d));
      break;
    case E::TAVG: da.addClimateData(ACD::tavg, kj::mv(d));
      break;
    case E::TMAX: da.addClimateData(ACD::tmax, kj::mv(d));
      break;
    case E::PRECIP: da.addClimateData(ACD::precip, kj::mv(d));
      break;
    case E::RELHUMID: da.addClimateData(ACD::relhumid, kj::mv(d));
      break;
    case E::WIND: da.addClimateData(ACD::wind, kj::mv(d));
      break;
    case E::GLOBRAD: da.addClimateData(ACD::globrad, kj::mv(d));
      break;
    default: ;
    }
  }
...
kj::Promise<J11Array> monica::fromCapnpSoilProfile(mas::schema::soil::Profile::Client profile) {
  return profile.dataRequest().send().then([](auto&& data) {
                                             ...
                                             for (const auto& layer : data.getLayers()) {
                                               ...
                                               double initialSoilMoisturePercent = -1.0;
                                               for (const auto& prop : layer.getProperties()) {
                                                 switch (prop.getName()) {
                                                 case mas::schema::soil::PropertyName::SAND: if (prop.
                                                     isF32Value())
                                                     l["Sand"] = prop.getF32Value() / 100.0;
                                                   break;
                                                 case mas::schema::soil::PropertyName::CLAY: if (prop.
                                                     isF32Value())
                                                     l["Clay"] = prop.getF32Value() / 100.0;
                                                   break;
                                                 case mas::schema::soil::PropertyName::SILT: if (prop.
                                                     isF32Value())
                                                     l["Silt"] = prop.getF32Value() / 100.0;
                                                   break;
                                                 case mas::schema::soil::PropertyName::ORGANIC_CARBON: if (prop.
                                                     isF32Value())
                                                     l["SoilOrganicCarbon"] = prop.getF32Value();
                                                   break;
                                                 case mas::schema::soil::PropertyName::ORGANIC_MATTER: if (prop.
                                                     isF32Value())
                                                     l["SoilOrganicMatter"] = prop.getF32Value() / 100.0;
                                                   break;
                                                 case mas::schema::soil::PropertyName::BULK_DENSITY: if (prop.
                                                     isF32Value())
                                                     l["SoilBulkDensity"] = prop.getF32Value();
                                                   break;
                                                 case mas::schema::soil::PropertyName::RAW_DENSITY: if (prop.
                                                     isF32Value())
                                                     l["SoilRawDensity"] = prop.getF32Value();
                                                   break;
                                                 case mas::schema::soil::PropertyName::P_H: if (prop.
                                                     isF32Value())
                                                     l["pH"] = prop.getF32Value();
                                                   break;
                                                 case mas::schema::soil::PropertyName::SOIL_TYPE: if (prop.
                                                     isType())
                                                     l["KA5TextureClass"] = prop.getType().cStr();
                                                   break;
                                                 case mas::schema::soil::PropertyName::PERMANENT_WILTING_POINT: if (prop
                                                     .isF32Value())
                                                     l["PermanentWiltingPoint"] = prop.getF32Value() / 100.0;
                                                   break;
                                                 case mas::schema::soil::PropertyName::FIELD_CAPACITY: if (prop.
                                                     isF32Value())
                                                     l["FieldCapacity"] = prop.getF32Value() / 100.0;
                                                   break;
                                                 case mas::schema::soil::PropertyName::SATURATION: if (prop.
                                                     isF32Value())
                                                     l["PoreVolume"] = prop.getF32Value() / 100.0;
                                                   break;
                                                 case
                                                 mas::schema::soil::PropertyName::SOIL_WATER_CONDUCTIVITY_COEFFICIENT
                                                 : if (prop.isF32Value()) l["Lambda"] = prop.getF32Value();
                                                   break;
                                                 case mas::schema::soil::PropertyName::SCELETON: if (prop.
                                                     isF32Value())
                                                 ...
                                                 : l["SoilAmmonium"] = prop.getF32Value();
                                                   break;
                                                 case mas::schema::soil::PropertyName::NITRATE: if (prop.
                                                     isF32Value())
                                                     l["SoilNitrate"] = prop.getF32Value();
                                                   break;
                                                 case mas::schema::soil::PropertyName::CN_RATIO: if (prop.
                                                     isF32Value())
                                                     l["CN"] = prop.getF32Value();
                                                   break;
                                                 case mas::schema::soil::PropertyName::SOIL_MOISTURE
                                                 : initialSoilMoisturePercent = prop.getF32Value();
                                                   break;
                                                 case mas::schema::soil::PropertyName::IN_GROUNDWATER: if (prop.
                                                     isBValue())
                                                     l["is_in_groundwater"] = prop.getBValue();
                                                   break;
                                                 case mas::schema::soil::PropertyName::IMPENETRABLE: if (prop.
                                                     isBValue())
                                                     l["is_impenetrable"] = prop.getBValue();
                                                   break;
                                                 }
                                               }
                                               if (initialSoilMoisturePercent >= 0.0 &&
                                                   l.find("FieldCapacity") != l.end()) {
                                                 // calculate actually the initial soil moisture in percent of field capacity
                                                 l["SoilMoisturePercentFC"] =
                                                   initialSoilMoisturePercent / (
                                                     l["FieldCapacity"].number_value() * 100.0);
                                               }
