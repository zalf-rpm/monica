...
int main(int argc, char **argv) {
  // #ifdef _WIN32
  //   cout << "PID: " << GetCurrentProcessId() << " - press ENTER to
  //   continue..."
  //        << endl;
  // #else
  //   cout << "PID: " << getpid() << " - press ENTER or SPACE to continue..."
  //        << endl;
  // #endif
  // cin.get();
  ...
  auto printHelp = [=]() {
    cout << appName << " [options] path-to-sim-json" << endl
         << endl
         << "options:" << endl
         << endl
         ...
         << " -sd  | --start-date ISO-DATE (default: start of given climate "
            "data) ... date in iso-date-format yyyy-mm-dd"
         << endl
         << " -ed  | --end-date ISO-DATE (default: end of given climate data) "
            "... date in iso-date-format yyyy-mm-dd"
         << endl
         << " -m   | --write-multiple-output-files ... write one output file "
            "per output section "
         << endl
         << " -op  | --path-to-output DIRECTORY (default: .) ... path to "
            "output directory"
         << endl
         << " -o   | --path-to-output-file FILE ... path to output file" << endl
         << " -c   | --path-to-crop FILE (default: ./crop.json) ... path to "
            "crop.json file"
         << endl
         << " -s   | --path-to-site FILE (default: ./site.json) ... path to "
            "site.json file"
         << endl
         << " -w   | --path-to-climate FILE (default: ./climate.csv) ... path "
            "to climate.csv"
         << endl;
  };
  ...
  if (argc > 1) {
    ...
    if (simj.failure())
      for (const auto &e : simj.errors)
        cerr << e << endl;
    auto simm = simj.result.object_items();

    auto csvos = simm["climate.csv-options"].object_items();
    if (!startDate.empty())
      csvos["start-date"] = startDate;
    if (!endDate.empty())
      csvos["end-date"] = endDate;
    simm["climate.csv-options"] = csvos;

    if (debugSet)
      simm["debug?"] = debug;

    // set debug mode in run-monica ... in libmonica has to be set separately in
    // runMonica
    ...
    if (simm["climate.csv"].is_string() &&
        simm["climate.csv"].string_value().find("capnp://") == string::npos) {
    ...
    if (icReaderSr.empty())
      icReaderSr = env.params.userCropParameters.pc_intercropping_reader_sr;
    if (icWriterSr.empty())
    ...
    if (writeMultipleOutputFiles) {
      ...
      if (k != string::npos)
        filenameWithoutExt = filename.substr(0, k);
    ...
    if (isIC && !isAsyncIC) {
      if (writeMultipleOutputFiles) {
        ...
        if (k != string::npos)
          filenameWithoutExt2 = filename2.substr(0, k);
