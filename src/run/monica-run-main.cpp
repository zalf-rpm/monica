...
int main(int argc, char** argv) {
  ...
  if (argc > 1) {
    for (auto i = 1; i < argc; i++) {
      string arg = argv[i];
      if (arg == "-d" || arg == "--debug") debug = debugSet = true;
      else if ((arg == "-sd" || arg == "--start-date") && i + 1 < argc) startDate = argv[++i];
      else if ((arg == "-ed" || arg == "--end-date") && i + 1 < argc) endDate = argv[++i];
      else if ((arg == "-op" || arg == "--path-to-output") && i + 1 < argc) pathToOutput = argv[++i];
      else if ((arg == "-o" || arg == "--path-to-output-file") && i + 1 < argc) pathToOutputFile = argv[++i];
      else if ((arg == "-o2" || arg == "--path-to-output-file2") && i + 1 < argc) pathToOutputFile2 = argv[++i];
      else if ((arg == "-m" || arg == "--write-multiple-output-files") && i + 1 <
               argc)
        writeMultipleOutputFiles = writeMultipleOutputFiles2 = true;
      else if ((arg == "-c" || arg == "--path-to-crop") && i + 1 < argc) crop = argv[++i];
      else if ((arg == "-s" || arg == "--path-to-site") && i + 1 < argc) site = argv[++i];
      else if ((arg == "-w" || arg == "--path-to-climate") && i + 1 < argc) climate = argv[++i];
      else if (arg == "-h" || arg == "--help") printHelp(), exit(0);
      else if (arg == "-v" || arg == "--version") cout << appName << " version " << version << endl, exit(0);
      else if ((arg == "-icrsr" || arg == "--intercropping-reader-sr") && i + 1 < argc) icReaderSr = argv[++i];
      else if ((arg == "-icwsr" || arg == "--intercropping-writer-sr") && i + 1 < argc) icWriterSr = argv[++i];
      else pathToSimJson = argv[i];
    }
    ...
    if (!pathToOutput.empty()) simm["path-to-output"] = pathToOutput;
    ...
    ps["crop"] = printPossibleErrors(parseJsonString(printPossibleErrors(readFile(simm["crop.json"].string_value()),
                                                                         activateDebug)), activateDebug);
    ps["site"] = printPossibleErrors(parseJsonString(printPossibleErrors(readFile(simm["site.json"].string_value()),
                                                                         activateDebug)), activateDebug);
    ...
    if (writeMultipleOutputFiles) {
      ...
      for (const auto& d : output.data) {
        if (writeOutputFile) {
          ...
          pathToOutputFile = fixSystemSeparator(pathToOutputDir + "/" + filenameWithoutExt + "_section_" +
      ...
      if (writeMultipleOutputFiles) {
        ...
        for (const auto& d : output2.data) {
          if (writeOutputFile) {
            ...
            sanitizedFileName = replace(sanitizedFileName, ">", "_rb_");
            sanitizedFileName = replace(sanitizedFileName, ":", "_colon_");
            pathToOutputFile = fixSystemSeparator(pathToOutputDir + "/" + filenameWithoutExt2 + "_2_section_" +
                                                  sanitizedFileName + ".csv");
            fout.open(pathToOutputFile);
            if (fout.fail()) {
              cerr << "Error while opening output file \"" << pathToOutputFile << "\"" << endl;
