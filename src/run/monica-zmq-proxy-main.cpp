...
int main(int argc, char **argv) {
  ...
  auto printHelp = [=]() {
    cout << appName << " [options] " << endl
         ...
         << endl
         << " -p | --pipeline-ports (use PULL/PUSH sockets for "
            "frontend/backend) ... deprecated"
         << endl
         << " -pps | --pull-push-sockets (use PULL/PUSH sockets for "
            "frontend/backend)"
         << endl
         << " -prs | --pull-router-sockets (use PULL/ROUTER sockets for "
            "frontend/backend)"
         << endl
         << " -fst | --frontend-socket-type FRONTEND_SOCKET_TYPE (default: "
            "ROUTER) ... use given frontend socket type"
         << endl
  ...
  for (auto i = 1; i < argc; i++) {
    string arg = argv[i];
    if ((arg == "-f" || arg == "--frontend-port") && i + 1 < argc)
    ...
    } else if (arg == "-p" || arg == "--pipeline-ports" || arg == "-pps" ||
               arg == "--pull-push-sockets")
