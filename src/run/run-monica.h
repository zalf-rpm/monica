...
CropRotation makeCropRotation(Tools::Date start, Tools::Date end,
                              std::vector<CultivationMethod> cropRotation);
...
struct Env {
  ...
  // 2nd priority, if climateData not valid, try to read climate data from csv
  // string

  std::vector<std::string> pathsToClimateCSV;
  // 3. priority, if climateCSV is empty, try to load climate data from vector
  // of file paths
