...
void monica::writeOutputHeaderRows(ostream &out, const vector<OId> &outputIds, string csvSep,
  ...
  for (auto oid : outputIds) {
    ...
    if (isOrgan)
      ...
      toLayer = fromLayer; // for aggregated ranges, which aren't being
                           // displayed as range
    ...
    for (int i = fromLayer; i <= toLayer; i++) {
      ...
      oss1 << (oss11.str().find_first_of(escapeTokens) == string::npos
                   ? oss11.str()
                   : "\""_s + oss11.str() + "\""_s)
           << csvSep_;
  ...
  if (!values.empty()) {
    for (size_t k = 0, size = values.begin()->size(); k < size; k++) {
      ...
      for (auto oid : outputIds) {
        ...
        switch (j.type()) {
        case Json::NUMBER:
          out << j.number_value() << csvSep_;
          break;
        case Json::STRING:
          out << (j.string_value().find_first_of(escapeTokens) == string::npos
                      ? j.string_value()
                      : "\""_s + j.string_value() + "\""_s)
              << csvSep_;
          break;
        case Json::BOOL:
          out << j.bool_value() << csvSep_;
          break;
        case Json::ARRAY: {
          size_t jvi = 0;
          ...
          for (Json jv : j.array_items()) {
            ...
            case Json::STRING:
              ...
              break;
            case Json::BOOL:
              out << jv.bool_value() << csvSep__;
              break;
            default:
              out << "UNKNOWN" << csvSep__;
            }
            ++jvi;
          }
          out << csvSep_;
...
void monica::writeOutputObj(ostream &out, const vector<OId> &outputIds,
                            const vector<J11Object> &values, string csvSep) {
  // using namespace std::string_literals;
  string escapeTokens = "\n\""_s + csvSep;
