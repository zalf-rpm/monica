...
Errors Soil::noSetPwpFcSat(SoilParameters *sp, int) {
  ...
  if (sp->vs_FieldCapacity < 0)
    errors.appendError("Field capacity not set!");
  if (sp->vs_Saturation < 0)
    errors.appendError("Saturation not set!");
  if (sp->vs_PermanentWiltingPoint < 0)
    errors.appendError("Permanent wilting point not set!");
  return errors;
}
...
void soilparameters::serialize(
  ...
  builder.setSoilStoneContent(sp->vs_SoilStoneContent);
  builder.setLambda(sp->vs_Lambda);
  builder.setFieldCapacity(sp->vs_FieldCapacity);
  builder.setSaturation(sp->vs_Saturation);
  builder.setPermanentWiltingPoint(sp->vs_PermanentWiltingPoint);
  builder.setSoilTexture(sp->vs_SoilTexture);
  builder.setSoilAmmonium(sp->vs_SoilAmmonium);
  builder.setSoilNitrate(sp->vs_SoilNitrate);
  builder.setSoilCNRatio(sp->vs_Soil_CN_Ratio);
  builder.setSoilMoisturePercentFC(sp->vs_SoilMoisturePercentFC);
  builder.setSoilRawDensity(sp->_vs_SoilRawDensity);
  builder.setSoilBulkDensity(sp->_vs_SoilBulkDensity);
  builder.setSoilOrganicCarbon(sp->_vs_SoilOrganicCarbon);
...
void soilparameters::deserialize(
  ...
  sp->vs_SoilStoneContent = reader.getSoilStoneContent();
  sp->vs_Lambda = reader.getLambda();
  sp->vs_FieldCapacity = reader.getFieldCapacity();
  sp->vs_Saturation = reader.getSaturation();
  sp->vs_PermanentWiltingPoint = reader.getPermanentWiltingPoint();
  sp->vs_SoilTexture = reader.getSoilTexture();
  sp->vs_SoilAmmonium = reader.getSoilAmmonium();
  sp->vs_SoilNitrate = reader.getSoilNitrate();
  sp->vs_Soil_CN_Ratio = reader.getSoilCNRatio();
  sp->vs_SoilMoisturePercentFC = reader.getSoilMoisturePercentFC();
  sp->_vs_SoilRawDensity = reader.getSoilRawDensity();
  sp->_vs_SoilBulkDensity = reader.getSoilBulkDensity();
  sp->_vs_SoilOrganicCarbon = reader.getSoilOrganicCarbon();
  sp->_vs_SoilOrganicMatter = reader.getSoilOrganicMatter();
}

Errors soilparameters::merge(SoilParameters *sp, json11::Json j) {
  Errors es;

  auto &vs_SoilSandContent = sp->vs_SoilSandContent;
  auto &vs_SoilClayContent = sp->vs_SoilClayContent;
  auto &vs_SoilpH = sp->vs_SoilpH;
  auto &vs_SoilStoneContent = sp->vs_SoilStoneContent;
  auto &vs_Lambda = sp->vs_Lambda;
  auto &vs_FieldCapacity = sp->vs_FieldCapacity;
  auto &vs_Saturation = sp->vs_Saturation;
  auto &vs_PermanentWiltingPoint = sp->vs_PermanentWiltingPoint;
  auto &vs_SoilTexture = sp->vs_SoilTexture;
  auto &vs_SoilAmmonium = sp->vs_SoilAmmonium;
  auto &vs_SoilNitrate = sp->vs_SoilNitrate;
  auto &vs_Soil_CN_Ratio = sp->vs_Soil_CN_Ratio;
  auto &vs_SoilMoisturePercentFC = sp->vs_SoilMoisturePercentFC;
  auto &_vs_SoilRawDensity = sp->_vs_SoilRawDensity;
  auto &_vs_SoilBulkDensity = sp->_vs_SoilBulkDensity;
  auto &_vs_SoilOrganicCarbon = sp->_vs_SoilOrganicCarbon;
  ...
  if (calculateAndSetPwpFcSat)
    es.append(calculateAndSetPwpFcSat(sp));
  ...
  //   es.appendError(kj::str("Sand content (", vs_SoilSandContent, ") is out of
  //   bounds [0, 1].").cStr());
  // }
...
json11::Json soilparameters::to_json(const SoilParameters *sp) {
  return J11Object{
      {"type", "SoilParameters"},
      {"Sand", J11Array{sp->vs_SoilSandContent, "% [0-1]"}},
      {"Clay", J11Array{sp->vs_SoilClayContent, "% [0-1]"}},
      {"pH", sp->vs_SoilpH},
      {"Sceleton", J11Array{sp->vs_SoilStoneContent, "vol% [0-1] (m3 m-3)"}},
      {"Lambda", sp->vs_Lambda},
...
void CapillaryRiseRates::addRate(const std::string &soilType, size_t distance,
  ...
  // >(bodart,std::pair<int,double>(distance,value)));
  ...
  if (!initialized) {
    ...
    if (!initialized) {
      ...
      try {
        ...
        cout << "Error: couldn't read CapillaryRiseRates.sercapnp or "
                "CapillaryRiseRates.json from folder "
             << pathToMonicaParamsSoil.toString().cStr()
             << " ! Exception: " << e.getDescription().cStr() << endl;
      }
      initialized = true;
    }
  }
...
bool soilparameters::isValid(const SoilParameters *sp) {
  bool is_valid = true;
  ...
  if (sp->_vs_SoilRawDensity<0) {
      cout << "SoilParameters::Error: Invalid soil raw density: "<<
  sp->_vs_SoilRawDensity << endl; is_valid = false;
  }
  */
  return is_valid;
}
...
double soilparameters::soilSiltContent(const SoilParameters *sp) {
...
Soil::createSoilPMs(const std::function<Errors(SoilParameters *)> &setPwpFcSat,
  ...
  for (const auto &sp : jsonSoilPMs) {
    auto sps = makeSoilParameters(setPwpFcSat);
    auto es = soilparameters::merge(&sps, sp);
    auto transf = transformIfNotMeters(sp, "Thickness");
      ...
      try {
        KJ_IF_MAYBE (file,
          ...
          errors.appendError(kj::str("Wessolek2009: Could neither load "
                                     "SoilCharacteristicData.sercapnp nor "
                                     "SoilCharacteristicData.json from folder ",
...
EResult<FcSatPwp> fcSatPwpFromKA5textureClass(const std::string &pathToSoilDir,
  ...
  if (texture.empty())
    return EResult<FcSatPwp>({}, "No soil texture given.");
  ...
  //  cout << "Saturation:\t\t" << vs_SaturationLowerBoundary << "\t" <<
  //  vs_SaturationUpperBoundary << endl; cout << "Field Capacity:\t" <<
  //  vs_FieldCapacityLowerBoundary << "\t" << vs_FieldCapacityUpperBoundary <<
  //  endl; cout << "PermanentWP:\t" << vs_PermanentWiltingPointLowerBoundary <<
  //  "\t" << vs_PermanentWiltingPointUpperBoundary << endl; cout << "Soil
  //  Organic Matter:\t" << vs_SoilOrganicMatter << endl;
  ...
  //   cout << "Saturation-Modifier:\t" << sat_mod_lowerBound << "\t" <<
  //   sat_mod_upperBound << endl; cout << "Field capacity-Modifier:\t" <<
  //   fc_mod_lowerBound << "\t" << fc_mod_upperBound << endl; cout <<
  //   "PWP-Modifier:\t" << pwp_mod_lowerBound << "\t" << pwp_mod_upperBound <<
  //   endl;

  // Linear interpolation
  double fcUnmod = fc_lowerBound;
  if (fc_upperBound < 0.5 && fc_lowerBound >= 1.0)
    fcUnmod = fc_lowerBound;
  else if (fc_lowerBound < 0.5 && fc_upperBound >= 1.0)
    fcUnmod = fc_upperBound;
  else if (srd_upperBound != srd_lowerBound) {
    fcUnmod = (srd - srd_lowerBound) / (srd_upperBound - srd_lowerBound) *
                  (fc_upperBound - fc_lowerBound) +
              fc_lowerBound;
  }

  double satUnmod = sat_lowerBound;
  if (sat_upperBound < 0.5 && sat_lowerBound >= 1.0)
    satUnmod = sat_lowerBound;
  else if (sat_lowerBound < 0.5 && sat_upperBound >= 1.0)
    satUnmod = sat_upperBound;
  else if (srd_upperBound != srd_lowerBound) {
    satUnmod = (srd - srd_lowerBound) / (srd_upperBound - srd_lowerBound) *
                   (sat_upperBound - sat_lowerBound) +
               sat_lowerBound;
  }

  double pwpUnmod = pwp_lowerBound;
  if (pwp_upperBound < 0.5 && pwp_lowerBound >= 1.0)
    pwpUnmod = pwp_lowerBound;
  else if (pwp_lowerBound < 0.5 && pwp_upperBound >= 1.0)
    pwpUnmod = pwp_upperBound;
  else if (srd_upperBound != srd_lowerBound) {
    pwpUnmod = (srd - srd_lowerBound) / (srd_upperBound - srd_lowerBound) *
                   (pwp_upperBound - pwp_lowerBound) +
               pwp_lowerBound;
  }
  ...
  if (som_upperBound != som_lowerBound) {
    fcMod = (som - som_lowerBound) / (som_upperBound - som_lowerBound) *
                (fc_mod_upperBound - fc_mod_lowerBound) +
            fc_mod_lowerBound;

    satMod = (som - som_lowerBound) / (som_upperBound - som_lowerBound) *
                 (sat_mod_upperBound - sat_mod_lowerBound) +
             sat_mod_lowerBound;

    pwpMod = (som - som_lowerBound) / (som_upperBound - som_lowerBound) *
                 (pwp_mod_upperBound - pwp_mod_lowerBound) +
             pwp_mod_lowerBound;
  }
...
FcSatPwp fcSatPwpFromVanGenuchtenVereecken(double sandFrac, double clayFrac,
  ...
  double fc_pF = 2.1;
  if (sandFrac > 0.48 && sandFrac <= 0.9 && clayFrac <= 0.12)
    fc_pF = 2.1 - (0.476 * (sandFrac - 0.48));
  else if (sandFrac > 0.9 && clayFrac <= 0.05)
    fc_pF = 1.9;
  else if (clayFrac > 0.45)
    fc_pF = 2.5;
  else if (clayFrac > 0.30 && sandFrac < 0.2)
    fc_pF = 2.4;
  else if (clayFrac > 0.35)
    fc_pF = 2.3;
  else if (clayFrac > 0.25 && sandFrac < 0.1)
    fc_pF = 2.3;
  else if (clayFrac > 0.17 && sandFrac > 0.68)
    fc_pF = 2.2;
  else if (clayFrac > 0.17 && sandFrac < 0.33)
    fc_pF = 2.2;
  else if (clayFrac > 0.08 && sandFrac < 0.27)
    fc_pF = 2.2;
  else if (clayFrac > 0.25 && sandFrac < 0.25)
    fc_pF = 2.2;
...
FcSatPwp fcSatPwpFromVanGenuchtenToth(bool isTopSoil, double sandFrac,
  ...
  //***** Van Genuchten Toth retention curve to calculate volumetric water
  // content at
  ...
  if (res.pwp <= pwpVgps.thetaR)
    res.pwp = fcVgps.thetaR + 0.01;
...
FcSatPwp fcSatPwpFromToth(double sandContent, double clayContent,
  ...
  // transform from [0 to 1] to [0 to 100] , in the future, I will change and
...
Errors updateUnsetPwpFcSatFromKA5textureClass(const std::string &pathToSoilDir,
                                              SoilParameters *sp) {
  if (sp->vs_SoilTexture.empty())
...
VanGenuchtenParams Soil::calcVanGenuchtenVereeckenParams(
  ...
  res.thetaR = pwp;
  res.thetaS = sat;
  res.alpha = exp(-2.486 + 2.5 * sandFrac - 35.1 * organicCarbonFrac -
...
VanGenuchtenParams Soil::calcVanGenuchtenTothParams(
  ...
  // cout << "PWP and FC are calculated with Toth-estimated van Genuchten
  // parameters" << endl;
  if (sandContentPerc >= 20)
    res.thetaR = 0.041;
...
std::function<Errors(SoilParameters *, int)>
