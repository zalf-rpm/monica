...
struct AOM_Properties {
  ...
  double vo_PartAOM_Slow_to_SMB_Slow{
      0.0}; //!< Partial transformation from AOM to SMB (soil microbiological biomass) for slow AOMs.
  double vo_PartAOM_Slow_to_SMB_Fast{
      0.0}; //!< Partial transformation from AOM to SMB (soil microbiological biomass) for fast AOMs.

  double vo_CN_Ratio_AOM_Slow{
      1.0}; //!< Used for calculation N-value if only C-value is known. Usually a constant value.
