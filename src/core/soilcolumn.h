 ...
 *
 * <img src="../../documentation/images/aom-diagramm.png" width="600" height="420">
 */
struct AOM_Properties {
  ...
  double vo_AOM_SlowDecCoeffStandard{1.0}; // Decomposition rate coefficient for slow AOM pool at
                                           // standard conditions
  double vo_AOM_FastDecCoeffStandard{1.0}; // Decomposition rate coefficient for fast AOM pool at
                                           // standard conditions

  double vo_PartAOM_Slow_to_SMB_Slow{0.0}; // Partial transformation from AOM to SMB (soil
                                           // microbiological biomass) for slow AOMs.
  double vo_PartAOM_Slow_to_SMB_Fast{0.0}; // Partial transformation from AOM to SMB (soil
                                           // microbiological biomass) for fast AOMs.

  double vo_CN_Ratio_AOM_Slow{1.0}; // Used for calculation N-value if only C-value is known.
...
void deserialize(AOM_Properties *aomp, mas::schema::model::monica::AOMProperties::Reader reader);
...
struct SoilLayer {
  ...
  double vs_FieldCapacity{-1.0};         // [m3 m-3]
  double vs_Saturation{-1.0};            // [m3 m-3]
  double vs_PermanentWiltingPoint{-1.0}; // [m3 m-3]
  ...
  double vs_SoilRawDensity{-1.0};    // [kg m-3]
  double vs_SoilBulkDensity{-1.0};   // [kg m-3]
  double vs_SoilOrganicCarbon{-1.0}; // [kg kg-1]
  double vs_SoilOrganicMatter{-1.0}; // [kg kg-1]
...
//! Resolved soil raw density (falls back to bulk density + clay content if
//! unset)
double soilRawDensity(const SoilLayer *sl);

//! Resolved soil bulk density (falls back to raw density + clay content if
//! unset)
double soilBulkDensity(const SoilLayer *sl);

//! Resolved soil organic carbon [kg C kg-1] (falls back to organic matter if
//! unset)
double soilOrganicCarbon(const SoilLayer *sl);

//! Resolved soil organic matter [kg OM kg-1] (falls back to organic carbon if
//! unset)
...
kj::Own<SoilColumn> makeSoilColumn(double layerThickness, double maxMineralisationDepth,
...
double applyMineralFertiliserViaNDemand(SoilColumn *sc, MineralFertilizerParameters fp,
...
inline size_t numberOfOrganicLayers(const SoilColumn *sc) { return sc->vs_NumberOfOrganicLayers; }
...
inline double layerThickness(const SoilColumn *sc) { return sc->layers.at(0).vs_LayerThickness; }
...
double applyMineralFertiliserViaNMinMethod(SoilColumn *sc,
                                           ...
                                           double samplingDepth, double cropNTargetValue,
