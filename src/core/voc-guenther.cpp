...
// kernels/mobile/modules/physiology/vocjjv/vocguenther.cpp
/*
 * @brief
 *  implemented by: Ruediger Grote (RG), IMK-IFU Garmisch-Partenkirchen,
 *      ruediger.grote@imk.fzk.de
 *  last changes:  22.08.2006 (bug fix and optional use of Guenther 1999 and
 * 2006) 22.02.2010 (considering Guenther et al. 1997 modification for Guenther
 * algorithm implementing co2 impact according to Wilkinson et al. 2009)
...
LeafEmissions calcLeafEmission(const leaf_emission_t &lemi,
  ...
  {
    // monoterpene, Guenther et al. (1993, 1995 (ctm), 1997 (factor 0.961, cit.
    // in Lindfors et al. 2000))
...
Voc::calculateGuentherVOCEmissionsMultipleSpecies(std::vector<SpeciesData> sds,
  ...
  for (const SpeciesData &species : sds) {
    if (species.mFol > 0.0) {
      ...
      // conversion of enzyme activity (umol m-2 s-1) in emission factor (ugC
      ...
      // emission in dependence on light and temperature, weighted over canopy
      // layers
      auto lems = calcLeafEmission(lemi, species.EF_MONOS);

      // conversion from (ugC g-1 h-1) to (umol m-2 s-1) and weighting with leaf
      // area and time
      ...
      // works only with 24 hour time step???        ph_.cUpt_vtfl[vt][fl] -=
      // ((ph_.ts_isoprene_emission_vtfl[vt][fl] * 5.0 +
      // ph_.ts_monoterpene_emission_vtfl[vt][fl] * 10.0) * MC / (UMOL_IN_MOL *
      // G_IN_KG));  // rg 18.06.10
