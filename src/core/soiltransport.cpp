...
void deserialize(SoilTransport *st,
                 mas::schema::model::monica::SoilTransportModuleState::Reader reader) {
  soiltransportmoduleparameters::deserialize(&st->modParams, reader.getModuleParams());
...
void serialize(const SoilTransport *st,
  ...
  soiltransportmoduleparameters::serialize(&st->modParams, builder.initModuleParams());
  setCapnpList(st->vq_Convection, builder.initConvection((capnp::uint)st->vq_Convection.size()));
  setCapnpList(st->vq_DiffusionCoeff,
               builder.initDiffusionCoeff((capnp::uint)st->vq_DiffusionCoeff.size()));
  setCapnpList(st->vq_Dispersion, builder.initDispersion((capnp::uint)st->vq_Dispersion.size()));
  setCapnpList(st->vq_DispersionCoeff,
  ...
  setCapnpList(st->vc_NUptakeFromLayer,
               builder.initVcNUptakeFromLayer((capnp::uint)st->vc_NUptakeFromLayer.size()));
  setCapnpList(st->vq_PoreWaterVelocity,
               builder.initPoreWaterVelocity((capnp::uint)st->vq_PoreWaterVelocity.size()));
  setCapnpList(st->vs_SoilMineralNContent,
               builder.initVsSoilMineralNContent((capnp::uint)st->vs_SoilMineralNContent.size()));
  ...
  setCapnpList(st->vq_TotalDispersion,
               builder.initTotalDispersion((capnp::uint)st->vq_TotalDispersion.size()));
  setCapnpList(st->vq_PercolationRate,
...
void step(SoilTransport *st) {
  ...
  for (size_t i = 0; i < nols; i++) {
    ...
    if (i == nols - 1) {
      ...
      st->vq_PercolationRate[i] = st->soilColumn->layers[i + 1].vs_SoilWaterFlux; //[mm]
    }
  ...
  for (int i = 0; i < nols; i++) {
    st->vq_SoilNO3[i] = st->vq_SoilNO3_aq[i] * st->soilColumn->layers[i].vs_SoilMoisture_m3;
...
void nDeposition(SoilTransport *st) {
  ...
  st->vq_SoilNO3[0] += dailyNDeposition / (10000.0 * st->soilColumn->layers[0].vs_LayerThickness);
}
...
void nUptake(SoilTransport *st) {
  ...
  for (size_t i = 0; i < nols; i++) {
    ...
    // content before movement of current time step (kg m soil-3 --> kg m
    // solute-3)
...
void nTransport(SoilTransport *st, double leachingDepth, double timeStepFactor) {
  double diffusionCoeffStandard = st->modParams.pq_DiffusionCoefficientStandard; // [m2 d-1]; old D0
  double AD = st->modParams.pq_AD; // Factor a in Kersebaum 1989 p.24 for Loess soils
  ...
  for (size_t i = 0; i < nols; i++) {
    ...
    if (i == 0) {
      ...
      if (pr >= 0.0 && wf0 >= 0.0) {
        st->vq_Convection[i] =
            (NO3 * pr) / lt; //[kg m-3] * [m t-1] / [m] // old KONV = Konvektion Diss S. 23
      } else if (pr >= 0 && wf0 < 0) {
      ...
      // layer > 0 && < bottom
      const double pr_o =
          st->vq_PercolationRate[i - 1] / 1000.0 * timeStepFactor; //[mm t-1 --> m t-1] * [t t-1]
      const double pr =
          st->vq_PercolationRate[i] / 1000.0 * timeStepFactor; // [mm t-1 --> m t-1] * [t t-1]
      const double NO3_u = st->vq_SoilNO3_aq[i + 1];

      if (pr >= 0.0 && pr_o >= 0.0) {
        const double NO3_o = st->vq_SoilNO3_aq[i - 1];
        st->vq_Convection[i] =
            ((NO3 * pr) - (NO3_o * pr_o)) / lt; // old KONV = Konvektion Diss S. 23
      } else if (pr >= 0 && pr_o < 0) {
      ...
      // bottom layer
      const double pr_o =
          st->vq_PercolationRate[i - 1] / 1000.0 * timeStepFactor; // [m t-1] * [t t-1]
      const double pr =
          st->soilColumn->vs_FluxAtLowerBoundary / 1000.0 * timeStepFactor; // [m t-1] * [t t-1]
  ...
  for (size_t i = 0; i < nols; i++) {
    ...
    st->vq_DiffusionCoeff[i] =
        ...
        (AD * exp(soilMoistureGradient[i] * 2.0 * 5.0) / soilMoistureGradient[i]) *
        timeStepFactor; //[m2 t-1] * [t t-1]

    // Dispersion coefficient, old DB
    if (i == 0) {
      st->vq_DispersionCoeff[i] =
          soilMoistureGradient[i] *
              (st->vq_DiffusionCoeff[i]                          // [m2 t-1]
      ...
      st->vq_DispersionCoeff[i] =
          soilMoistureGradient[i] *
              (st->vq_DiffusionCoeff[i] + dispersionLength * st->vq_PoreWaterVelocity[i]) -
          (0.5 * lti * fabs(pri)) +
          ((0.5 * st->envParams->p_timeStep * timeStepFactor * fabs((pri + pr_o) / 2.0)) *
    ...
    if (i == 0) {
      const double NO3_u = st->vq_SoilNO3_aq[i + 1];
      // vq_Dispersion = Dispersion upwards or downwards, depending on the
      // position in the profile [kg m-3]
      st->vq_Dispersion[i] =
          -st->vq_DispersionCoeff[i] * (NO3i - NO3_u) / (lti * lti); // [m2] * [kg m-3] / [m2]
    } else if (i < nols - 1) {
      const double NO3_o = st->vq_SoilNO3_aq[i - 1];
      const double NO3_u = st->vq_SoilNO3_aq[i + 1];
  ...
  if (st->vq_PercolationRate[leachingDepthLayerIndex] > 0.0) {
    ...
    const auto lt = st->soilColumn->layers[leachingDepthLayerIndex].vs_LayerThickness;
    const auto NO3 = st->vq_SoilNO3_aq[leachingDepthLayerIndex];

    if (leachingDepthLayerIndex < nols - 1) {
      const double pr_u =
          st->vq_PercolationRate[leachingDepthLayerIndex + 1] / 1000.0 * timeStepFactor; // [m t-1]
      ...
      st->vq_LeachingAtBoundary +=
          ...
          ((st->vq_DispersionCoeff[leachingDepthLayerIndex] * (NO3 - NO3_u)) / (lt * lt) * 10000.0 *
           lt); //[kg ha-1]
    } else {
      const double pr_u =
          st->soilColumn->vs_FluxAtLowerBoundary / 1000.0 * timeStepFactor; // [m t-1]
    ...
    if (leachingDepthLayerIndex < nols - 1) {
      ...
      st->vq_LeachingAtBoundary += ((pr_u * NO3_u) / (lt * 10000.0 * lt)) +
                                   st->vq_DispersionCoeff[leachingDepthLayerIndex] * (NO3 - NO3_u) /
...
void putCrop(SoilTransport *st, CropModule *cm) { st->cropModule = cm; }

void removeCrop(SoilTransport *st) { st->cropModule = nullptr; }
