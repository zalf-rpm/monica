...
void serialize(
  ...
  setCapnpList(
      st->vq_DiffusionCoeff,
      builder.initDiffusionCoeff((capnp::uint)st->vq_DiffusionCoeff.size()));
  setCapnpList(st->vq_Dispersion,
               builder.initDispersion((capnp::uint)st->vq_Dispersion.size()));
  setCapnpList(
  ...
  setCapnpList(st->vq_PoreWaterVelocity,
               builder.initPoreWaterVelocity(
                   (capnp::uint)st->vq_PoreWaterVelocity.size()));
  setCapnpList(st->vs_SoilMineralNContent,
               builder.initVsSoilMineralNContent(
                   (capnp::uint)st->vs_SoilMineralNContent.size()));
  setCapnpList(st->vq_SoilNO3,
...
void nTransport(SoilTransport *st, double leachingDepth,
  ...
  for (size_t i = 0; i < nols; i++) {
    ...
    if (i == 0) {
      const double pr = st->vq_PercolationRate[i] / 1000.0 *
                        timeStepFactor; // [mm t-1 --> m t-1]
      const double NO3_u = st->vq_SoilNO3_aq[i + 1];

      if (pr >= 0.0 && wf0 >= 0.0) {
        st->vq_Convection[i] =
            (NO3 * pr) /
            lt; //[kg m-3] * [m t-1] / [m] // old KONV = Konvektion Diss S. 23
      } else if (pr >= 0 && wf0 < 0) {
      ...
      // layer > 0 && < bottom
      const double pr_o = st->vq_PercolationRate[i - 1] / 1000.0 *
      ...
      if (pr >= 0.0 && pr_o >= 0.0) {
        ...
        st->vq_Convection[i] = ((NO3 * pr) - (NO3_o * pr_o)) /
                               lt; // old KONV = Konvektion Diss S. 23
      } else if (pr >= 0 && pr_o < 0) {
      ...
      // bottom layer
      const double pr_o = st->vq_PercolationRate[i - 1] / 1000.0 *
                          timeStepFactor; // [m t-1] * [t t-1]
      const double pr = st->soilColumn->vs_FluxAtLowerBoundary / 1000.0 *
                        timeStepFactor; // [m t-1] * [t t-1]

      if (pr >= 0.0 && pr_o >= 0.0) {
        const double NO3_o = st->vq_SoilNO3_aq[i - 1];
        st->vq_Convection[i] =
            ((NO3 * pr) - (NO3_o * pr_o)) / lt; // KONV = Konvektion Diss S. 23
      } else if (pr >= 0 && pr_o < 0) {
        st->vq_Convection[i] = ((NO3 * pr) - (NO3 * pr_o)) / lt;
      } else if (pr < 0 && pr_o < 0) {
  ...
  for (size_t i = 0; i < nols; i++) {
    ...
    st->vq_DiffusionCoeff[i] = diffusionCoeffStandard *
    ...
    if (i == 0) {
      ...
      const double pr_o =
          st->vq_PercolationRate[i - 1] / 1000.0 * timeStepFactor; // [m t-1]
    ...
    if (i == 0) {
      ...
      st->vq_Dispersion[i] = -st->vq_DispersionCoeff[i] * (NO3i - NO3_u) /
                             (lti * lti); // [m2] * [kg m-3] / [m2]
    } else if (i < nols - 1) {
      const double NO3_o = st->vq_SoilNO3_aq[i - 1];
      const double NO3_u = st->vq_SoilNO3_aq[i + 1];
      st->vq_Dispersion[i] =
  ...
  if (st->vq_PercolationRate[leachingDepthLayerIndex] > 0.0) {
    ...
    if (leachingDepthLayerIndex < nols - 1) {
      const double pr_u = st->vq_PercolationRate[leachingDepthLayerIndex + 1] /
                          1000.0 * timeStepFactor; // [m t-1]
      const double NO3_u =
          st->vq_SoilNO3_aq[leachingDepthLayerIndex + 1]; // [kg m-3]
      ...
      const double pr_u = st->soilColumn->vs_FluxAtLowerBoundary / 1000.0 *
                          timeStepFactor;                          // [m t-1]
