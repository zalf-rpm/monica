...
double hPhoto::diffuse_fraction_cscor(double diffuse_fraction, double solar_elevation)
{
  ...
  /*if ((solar_elevation < 0) && (solar_elevation > (0.5 * M_PI)))  // ((solar_elevation < 0 - epsilon) && (solar_elevation > (0.5 * pi) + epsilon))
  {
...
double hPhoto::Spitters_canop_photo_dL(double beta, double L, double I0_dr, double I0_df, double A_m, double epsilon, double k_df, double sigma, int leaf_angle_integration_style)
{
  //beta *= deg2rad               // convert [deg] to [rad] if input is in [deg]
  double sinbeta = sin(beta);   // beta is the solar elevation angle [rad]
  ...
  double first = (1 - pow(1 - sigma, 0.5)) / (1 + pow(1 - sigma, 0.5)); // reflection of a canopy of horizontal leaves (Goudriaan, 1977, p. 14,31, cited by Spitters 1986)
  double second = 2 / (1 + 1.6 * sinbeta);                              // approximate correction factor for a spherical leaf angle distribution (Goudriaan, personal communication, cited by Spitters 1986)
  ...
  */
  ...
  double k_dr = k_bl * pow(1.0 - sigma, 0.5);                 // extinction coefficient for total direct PAR flux; non-black leaves correction from Spitters et al. (1989)
  ...
  double Ia_dr = (1 - rho) * I0_dr * pow(1 - sigma, 0.5) * k_dr * exp(- pow(1 - sigma, 0.5) * k_dr * L);    // eq. 11, but using k_dr from Spitters et al. (1989)
  ...
  if (leaf_angle_integration_style == 0) {  // None
    ...
    if (leaf_angle_integration_style == 1) {        // Spitters 1986, custom implementation including Wageningen school implementations-inspired numerical safeguards;
      ...
      for (int i = 0; i < gaussian_distances.size(); ++i) {
...
double hPhoto::Spitters_canop_photo_multilayer(double beta, double LAI, double I0_dr, double I0_df, double A_m, double epsilon, double k_df, double sigma, int n_canopy_layers, int leaf_angle_integration_style) {
  ...
  for (int i = 0; i < n_canopy_layers; ++i) {
    ...
    auto A = Spitters_canop_photo_dL(beta, L, I0_dr, I0_df, A_m, epsilon, k_df, sigma, leaf_angle_integration_style);
...
double hPhoto::Spitters_canop_photo_3p(double beta, double LAI, double I0_dr, double I0_df, double A_m, double epsilon, double k_df, double sigma, int leaf_angle_integration_style) {
  // 3-point integration procedure from Spitters (1986)
  // eq. 20, eq. 21
  // L = (0.5 + p * pow(0.15, 0.5)) * LAI;               // p = 1,0,1
  // A_h = LAI * _A(_L) + 1.6*_A_(_L_) + A_(L_)) / 3.6;

  vector<double> canopy_layers = {0.112702, 0.5, 0.887298};
  vector<double> gaussian_weights = {0.277778, 0.444444, 0.277778};

  // selection of canopy depths (LAIC from top)
  double A_canop = 0., L, A = 0.;

  for (int l = 0; l < canopy_layers.size(); ++l) {
    ...
    A = Spitters_canop_photo_dL(beta, L, I0_dr, I0_df, A_m, epsilon, k_df, sigma, leaf_angle_integration_style);
