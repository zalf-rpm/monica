...
struct FvCB_canopy_hourly_out {
  double canopy_net_photos;   // [μmol CO2 m-2 h-1]
  double canopy_resp;         // [μmol CO2 m-2 h-1]
  double canopy_gross_photos; // [μmol CO2 m-2 h-1]
  double jmax_c;              // ??? [μmol m-2 s-1] ?
 ...
 * @param Jmax_25 actual electron transport capacity (unit leaf area) at 25°C [μmol m-2 s-1] 
 * @return double 
 */
