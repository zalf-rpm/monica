...
double inline stepwiseLinearFunction3(double x,
  ...
  return (ymax - ymin) / (xmax - xmin) * (x - xmin);
}
...
inline double stepwiseLinearFunction4(double x, 
                                      ...
                                      double ymax = 1.0) {
  if (x <= xmin) return ymin;
  if (xmin < x && x < x1) return (ymax - ymin) / (x1 - xmin) * (x - xmin);
...
double fWFPS(double swc, double hminn, double hoptn, double fc, double sat) {
  return stepwiseLinearFunction4(swc, hminn, hoptn, fc, sat);
}
...
double nitrification(const StixParameters& ps, 
  ...
  return 
    ...
    * fTres
    * nit::fWFPS(soilwaterContent, ps.hminn, ps.hoptn, fc, sat);
}
