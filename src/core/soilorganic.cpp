...
void SoilOrganic::serialize(mas::schema::model::monica::SoilOrganicModuleState::Builder builder) const {
  ...
  setCapnpList(vo_ActAmmoniaOxidationRate, builder.initActAmmoniaOxidationRate((uint)vo_ActAmmoniaOxidationRate.size()));
  setCapnpList(vo_ActNitrificationRate, builder.initActNitrificationRate((uint)vo_ActNitrificationRate.size()));
  setCapnpList(vo_ActDenitrificationRate, builder.initActDenitrificationRate((uint)vo_ActDenitrificationRate.size()));
  setCapnpList(vo_AOM_FastDeltaSum, builder.initAomFastDeltaSum((uint)vo_AOM_FastDeltaSum.size()));
  setCapnpList(vo_AOM_FastInput, builder.initAomFastInput((uint)vo_AOM_FastInput.size()));
  setCapnpList(vo_AOM_FastSum, builder.initAomFastSum((uint)vo_AOM_FastSum.size()));
  setCapnpList(vo_AOM_SlowDeltaSum, builder.initAomSlowDeltaSum((uint)vo_AOM_SlowDeltaSum.size()));
  setCapnpList(vo_AOM_SlowInput, builder.initAomSlowInput((uint)vo_AOM_SlowInput.size()));
  setCapnpList(vo_AOM_SlowSum, builder.initAomSlowSum((uint)vo_AOM_SlowSum.size()));
  setCapnpList(vo_CBalance, builder.initCBalance((uint)vo_CBalance.size()));
  builder.setDecomposerRespiration(vo_DecomposerRespiration);
  builder.setErrorMessage(vo_ErrorMessage);
  setCapnpList(vo_InertSoilOrganicC, builder.initInertSoilOrganicC((uint)vo_InertSoilOrganicC.size()));
  ...
  setCapnpList(vo_NetNMineralisationRate, builder.initNetNMineralisationRate((uint)vo_NetNMineralisationRate.size()));
  builder.setTotalNH3Volatilised(vo_Total_NH3_Volatilised);
  builder.setNh3Volatilised(vo_NH3_Volatilised);
  setCapnpList(vo_SMB_CO2EvolutionRate, builder.initSmbCO2EvolutionRate((uint)vo_SMB_CO2EvolutionRate.size()));
  setCapnpList(vo_SMB_FastDelta, builder.initSmbFastDelta((uint)vo_SMB_FastDelta.size()));
  setCapnpList(vo_SMB_SlowDelta, builder.initSmbSlowDelta((uint)vo_SMB_SlowDelta.size()));
  setCapnpList(vs_SoilMineralNContent, builder.initVsSoilMineralNContent((uint)vs_SoilMineralNContent.size()));
  setCapnpList(vo_SoilOrganicC, builder.initSoilOrganicC((uint)vo_SoilOrganicC.size()));
  setCapnpList(vo_SOM_FastDelta, builder.initSomFastDelta((uint)vo_SOM_FastDelta.size()));
  setCapnpList(vo_SOM_FastInput, builder.initSomFastInput((uint)vo_SOM_FastInput.size()));
  setCapnpList(vo_SOM_SlowDelta, builder.initSomSlowDelta((uint)vo_SOM_SlowDelta.size()));
