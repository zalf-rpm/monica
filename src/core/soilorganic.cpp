...
void foUrea(SoilOrganic *so) {
  ...
  std::vector<double> vo_SoilCarbamid_aq(nools,
                                         0.0); // Dissolved carbamide concetzration in soil solution
                                               // [kmol urea m-3]
  ...
  for (int i = 0; i < so->soilColumn.vs_NumberOfOrganicLayers; i++) {
    ...
    // kmol urea m-3 soil
    vo_SoilCarbamid_solid[i] = layer.vs_SoilCarbamid / OrganicConstants::po_UreaMolecularWeight /
    ...
    // kmol urea m-3 soil
    vo_SoilCarbamid_aq[i] = (vo_SoilCarbamid_aq[i] / (1.0 + (vo_SoilCarbamid_aq[i] * 0.0453))) *
    ...
    vo_HydrolysisRate1[i] = (po_HydrolysisP1 * (soillayer::soilOrganicMatter(&layer) * 100.0) *
                                 OrganicConstants::po_SOM_to_C +
                             po_HydrolysisP2) /
                            OrganicConstants::po_UreaMolecularWeight;

    vo_HydrolysisRate2[i] = vo_HydrolysisRate1[i] / (exp(-po_ActivationEnergy / (8.314 * 310.0)));

    vo_HydrolysisRateMax[i] =
        vo_HydrolysisRate2[i] *
        exp(-po_ActivationEnergy / (8.314 * (layer.vs_SoilTemperature + 273.15)));

    vo_Hydrolysis_pH_Effect[i] = exp(-0.064 * ((layer.vs_SoilpH - 6.5) * (layer.vs_SoilpH - 6.5)));
    ...
    vo_HydrolysisRate[i] = vo_HydrolysisRate[i] * 86400.0 * soillayer::soilBulkDensity(&layer);
    ...
    if (vo_HydrolysisRate[i] >= vo_SoilCarbamid_aq[i]) {
      ...
      // kg N m soil-3
      layer.vs_SoilCarbamid -= vo_HydrolysisRate[i] * OrganicConstants::po_UreaMolecularWeight *
                               OrganicConstants::po_Urea_to_N * 1000.0;

      // kg N m soil-3
      layer.vs_SoilNH4 += vo_HydrolysisRate[i] * OrganicConstants::po_UreaMolecularWeight *
                          OrganicConstants::po_Urea_to_N * 1000.0;
    }
    ...
    if (i == 0) {
      ...
      vo_NH3aq_EquilibriumConst = pow(10.0, ((-2728.3 / (layer0.vs_SoilTemperature + 273.15)) -
                                             0.094219)); // K2 in Sadeghi's program

      vo_NH3_EquilibriumConst = pow(10.0, ((1630.5 / (layer0.vs_SoilTemperature + 273.15)) -
...
void step(SoilOrganic *so, double meanAirTemperature, double precipitation, double windSpeed) {
  ...
  auto n2OProducedNitDenit = so->params.sticsParams.use_n2o ? foSticsN2OProduction(so)
...
void addOrganicMatter(SoilOrganic *so, const OrganicMatterParameters &params,
  ...
  debug() << "SoilOrganic: addOrganicMatter: " << organicmatterparameters::to_json(&params).dump()
          << endl;
  ...
  auto calc_CN_Ratio_AOM_Fast_and_added_Corg_amount =
      [&params, layerThickness, so](double vo_AddedOrganicMatterAmount,
                                    double vo_AddedOrganicMatterNConcentration) {
        double CN_ratio_AOM_fast = 0;

        double added_Corg_amount =
            (params.vo_CorgContent <= 0.0 ? OrganicConstants::po_AOM_to_C : params.vo_CorgContent) *
            vo_AddedOrganicMatterAmount * params.vo_AOM_DryMatterContent / 10000.0 / layerThickness;

        double added_Norg_amount =
            vo_AddedOrganicMatterNConcentration <= 0.0
                ? 0.01
                : (vo_AddedOrganicMatterAmount * params.vo_AOM_DryMatterContent *
                   vo_AddedOrganicMatterNConcentration / 10000.0 / layerThickness);

        debug() << "Added organic matter N amount: " << added_Norg_amount << endl;

        double N_for_AOM_slow =
  ...
  auto areSameAOMPropsAsOMParams = [=](const AOM_Properties &props) {
    return roundShiftedInt(props.vo_AOM_SlowDecCoeffStandard, 4) ==
           ...
           roundShiftedInt(props.vo_CN_Ratio_AOM_Slow, 4) == rounded_CN_Ratio_AOM_Slow;
  };
    ...
    for (const auto &p : layer2addedOrganicMatterAmount) {
      if (p.first < nools) {
        so->soilColumn.layers.at(p.first).vs_SoilCarbamid +=
            p.second * params.vo_AOM_DryMatterContent * params.vo_AOM_CarbamidContent / 10000.0 /
            layerThickness;
      }
    }
  }
  ...
  for (const auto &[intoLayerIndex, addedOrganicMatterAmount] : layer2addedOrganicMatterAmount) {
    ...
    if (poolSetIndex < 0) {
      ...
      cpool.vo_AOM_Slow += AOM_slow_input = params.vo_PartAOM_to_AOM_Slow * added_Corg_amount;
      double added_CN_ratio_AOM_fast =
          areCropResidueParams ? calced_CN_Ratio_AOM_Fast : params.vo_CN_Ratio_AOM_Fast;
    ...
    double soil_NH4_input = params.vo_AOM_NH4Content * addedOrganicMatterAmount *
                            params.vo_AOM_DryMatterContent / 10000.0 / layerThickness;

    double soil_NO3_input = params.vo_AOM_NO3Content * addedOrganicMatterAmount *
                            params.vo_AOM_DryMatterContent / 10000.0 / layerThickness;
  ...
  for (size_t i = 0; i < so->vs_NumberOfOrganicLayers; i++) {
    ...
    if (layi.vs_Soil_CN_Ratio > 100) {
      so->vo_InertSoilOrganicC_highCN[i] =
          ...
          (po_inert_CN_lower_limit / po_inert_CN_upper_limit - 1) / 10000.0 * 1000.0 /
      ...
      so->vo_InertSoilOrganicC[i] =
          (0.049 *
           pow((so->vo_SoilOrganicC_highCN[i] * layer.vs_LayerThickness / 1000 * 10000.0), 1.139)) /
      ...
      so->vo_InertSoilOrganicC[i] =
          (0.049 *
           pow((so->vo_SoilOrganicC[i] * layer.vs_LayerThickness / 1000 * 10000.0), 1.139)) /
          10000.0 * 1000.0 / layer.vs_LayerThickness;
    }
    ...
    layer.vs_SOM_Slow = so->vo_SoilOrganicC[i] /
    ...
    layer.vs_SoilOrganicCarbon =
        (so->vo_SoilOrganicC[i] + so->vo_InertSoilOrganicC[i]) / soillayer::soilBulkDensity(&layer);
...
void foMIT(SoilOrganic *so) {
  ...
  double po_SOM_SlowUtilizationEfficiency = params.po_SOM_SlowUtilizationEfficiency;
  double po_SOM_FastUtilizationEfficiency = params.po_SOM_FastUtilizationEfficiency;
  ...
  double po_AOM_SlowUtilizationEfficiency = params.po_AOM_SlowUtilizationEfficiency;
  double po_AOM_FastUtilizationEfficiency = params.po_AOM_FastUtilizationEfficiency;
  ...
  for (int i = 0; i < nools; i++) {
    ...
    double tod = params.__enable_kaiteew_TempOnDecompostion__
                     ? foTempOnDecompostionKaiteew(so, layi.vs_SoilTemperature,
                                                   params.po_QTenFactor, params.po_TempDecOptimal)
                     : foTempOnDecompostion(so, layi.vs_SoilTemperature); // prev code

    double mod = params.__enable_kaiteew_MoistOnDecompostion__
                     ? foMoistOnDecompostionKaiteew(so, layi.vs_SoilMoisture_m3, layi.vs_Saturation,
    ...
    double cod =
        params.__enable_kaiteew_ClayOnDecompostion__
            ? foClayOnDecompostionKaiteew(so, layi.vs_SoilClayContent, params.po_LimitClayEffect)
    ...
    vo_SMB_SlowMaintRateCoeff[i] = po_SMB_SlowMaintRateStandard * cod * tod * mod;
  ...
  for (int i = 0; i < nools; i++) {
    ...
    vo_SMB_FastDelta[i] = (po_SMB_UtilizationEfficiency * (1.0 - po_PartSMB_Slow_to_SOM_Fast) *
  ...
  for (int i = 0; i < nools; i++) {
    ...
    vo_NBalance[i] =
        -(vo_SMB_SlowDelta[i] / po_CN_Ratio_SMB) - (vo_SMB_FastDelta[i] / po_CN_Ratio_SMB) -
        (vo_SOM_SlowDelta[i] / CN_Ratio_SOM_Slow) - (vo_SOM_FastDelta[i] / CN_Ratio_SOM_Fast);
  ...
  for (int i = 0; i < nools; i++) {
    ...
    if (vo_NBalance[i] < 0.0) {
      if (fabs(vo_NBalance[i]) >= ((layi.vs_SoilNH4 * po_ImmobilisationRateCoeffNH4) +
        ...
        if (fabs(vo_NBalance[i]) >= (layi.vs_SoilNH4 * po_ImmobilisationRateCoeffNH4)) {
          layi.vs_SoilNO3 -=
              fabs(vo_NBalance[i]) - (layi.vs_SoilNH4 * po_ImmobilisationRateCoeffNH4);
 ...
 * @param vw_WindSpeed
 */
void foVolatilisation(SoilOrganic *so, bool vo_AOM_Addition, double vw_MeanAirTemperature,
                      double vw_WindSpeed) {
  double vo_SoilWet;
  ...
  if (vo_DaysAfterApplicationSum > 0 || vo_AOM_Addition) {
    ...
    for (auto &props : AOM_Pool) {
      ...
      vo_MaxVolatilisation =
          0.0495 * pow(1.1020, vo_SoilWet) * pow(1.0223, vw_MeanAirTemperature) *
          pow(1.0417, vw_WindSpeed) * pow(1.1080, props.vo_AOM_DryMatterContent) *
          pow(0.8280, vo_AOM_TAN_Content) * pow(11.300, double(props.incorporation));

      vo_VolatilisationHalfLife =
          1.0380 * pow(1.1020, vo_SoilWet) * pow(0.9600, vw_MeanAirTemperature) *
          pow(0.9500, vw_WindSpeed) * pow(1.1750, props.vo_AOM_DryMatterContent) *
          pow(1.1060, vo_AOM_TAN_Content) * pow(1.0000, double(props.incorporation)) *
      ...
      vo_VolatilisationRate =
          vo_MaxVolatilisation *
          (vo_VolatilisationHalfLife /
           (pow((props.vo_DaysAfterApplication + vo_VolatilisationHalfLife), 2.0)));

      vo_N_PotVolatilised = vo_VolatilisationRate * vo_AOM_TAN_Content *
                            (props.vo_AOM_Slow + props.vo_AOM_Fast) / 10000.0 / 1000.0;
...
void foNitrification(SoilOrganic *so) {
  ...
  auto nools = soilColumn.vs_NumberOfOrganicLayers;
  double po_AmmoniaOxidationRateCoeffStandard = params.po_AmmoniaOxidationRateCoeffStandard;
  double po_NitriteOxidationRateCoeffStandard = params.po_NitriteOxidationRateCoeffStandard;
  ...
  for (int i = 0; i < nools; i++) {
    ...
    vo_AmmoniaOxidationRateCoeff[i] = po_AmmoniaOxidationRateCoeffStandard *
    ...
    vo_NitriteOxidationRateCoeff[i] = po_NitriteOxidationRateCoeffStandard *
                                      foTempOnNitrification(so, layi.vs_SoilTemperature) *
                                      foMoistOnNitrification(so, soillayer::soilMoisturePF(&layi)) *
...
void foDenitrification(SoilOrganic *so) {
  ...
  for (int i = 0; i < nools; i++) {
    ...
    vo_PotDenitrificationRate[i] = po_SpecAnaerobDenitrification * vo_SMB_CO2EvolutionRate[i] *
                                   foTempOnNitrification(so, layi.vs_SoilTemperature);

    vo_ActDenitrificationRate[i] =
        min(vo_PotDenitrificationRate[i] *
                foMoistOnDenitrification(so, layi.vs_SoilMoisture_m3, layi.vs_Saturation),
    ...
    vo_TotalDenitrification +=
        vo_ActDenitrificationRate[i] * layi.vs_LayerThickness; // [kg m-3] --> [kg m-2] ;
  }
...
double foN2OProduction(SoilOrganic *so) {
  ...
  for (int i = 0; i < nools; i++) {
    ...
    double N2OProductionAtLayer =
        NO2i * foTempOnNitrification(so, tempi) * N2OProductionRate * pH_response * lti *
...
void foPoolUpdate(SoilOrganic *so) {
  ...
  for (int i = 0; i < nools; i++) {
    ...
    vo_CBalance[i] = vo_AOM_SlowInput[i] + vo_AOM_FastInput[i] + vo_AOM_SlowDeltaSum[i] +
    ...
    vo_SoilOrganicC[i] = (soillayer::soilOrganicCarbon(&layi) * soillayer::soilBulkDensity(&layi)) -
                         vo_InertSoilOrganicC[i];
    vo_SoilOrganicC[i] += vo_CBalance[i];

    // [kg C m-3] / [kg m-3] --> [kg C kg-1]
    layi.vs_SoilOrganicCarbon =
        (vo_SoilOrganicC[i] + vo_InertSoilOrganicC[i]) / soillayer::soilBulkDensity(&layi);
...
double foClayOnDecompostionKaiteew(SoilOrganic *so, double d_SoilClayContent,
  ...
  if (d_SoilClayContent >= 0.0 && d_SoilClayContent <= 1.0) {
    clayOnDecomposition =
        (1.0 - d_LimitClayEffect) / (1.0 + exp(-3.14 + d_SoilClayContent * 16)) + d_LimitClayEffect;
  } else {
 ...
 * @return
 */
double foClayOnDecompostion(SoilOrganic *so, double d_SoilClayContent, double d_LimitClayEffect) {
 ...
 */
double foTempOnDecompostionKaiteew(SoilOrganic *so, double soilTemperature, double QTenFactor,
                                   double tempDecOptimal) {
  double tempOnDecomposition = 0.0;

  if (soilTemperature > 0.0 && soilTemperature <= 100.0) {
    tempOnDecomposition = 1.0 /
 ...
 */
double foMoistOnDecompostionKaiteew(SoilOrganic *so, double d_SoilMoisture_m3, double d_Saturation,
  ...
  if (d_SoilMoisture_m3 / d_Saturation >= 0.0 && d_SoilMoisture_m3 / d_Saturation <= 1.0) {
    moistOnDecomposition =
        exp(-18 * pow((d_SoilMoisture_m3 / d_Saturation - d_MoistureDecOptimal), 2));
  } else {
 ...
 * @return
 */
double foMoistOnDenitrification(SoilOrganic *so, double d_SoilMoisture_m3, double d_Saturation) {
 ...
 * @return
 */
double foNH3onNitriteOxidation(SoilOrganic *so, double d_SoilNH4, double d_SoilpH) {
  ...
  fo_NH3onNitriteOxidation =
      po_Inhibitor_NH3 /
      (po_Inhibitor_NH3 +
       d_SoilNH4 * (1 - 1 / (1.0 + pow(10.0, d_SoilpH - OrganicConstants::po_pKaNH3))));

  return fo_NH3onNitriteOxidation;
}

void deserialize(SoilOrganic *so,
...
void serialize(const SoilOrganic *so,
  ...
  setCapnpList(
      so->vo_ActAmmoniaOxidationRate,
      builder.initActAmmoniaOxidationRate((capnp::uint)so->vo_ActAmmoniaOxidationRate.size()));
  setCapnpList(so->vo_ActNitrificationRate,
               builder.initActNitrificationRate((capnp::uint)so->vo_ActNitrificationRate.size()));
  setCapnpList(
      so->vo_ActDenitrificationRate,
      builder.initActDenitrificationRate((capnp::uint)so->vo_ActDenitrificationRate.size()));
  setCapnpList(so->vo_AOM_FastDeltaSum,
               builder.initAomFastDeltaSum((capnp::uint)so->vo_AOM_FastDeltaSum.size()));
  setCapnpList(so->vo_AOM_FastInput,
  ...
  setCapnpList(so->vo_AOM_SlowDeltaSum,
               builder.initAomSlowDeltaSum((capnp::uint)so->vo_AOM_SlowDeltaSum.size()));
  setCapnpList(so->vo_AOM_SlowInput,
  ...
  setCapnpList(so->vo_InertSoilOrganicC,
               builder.initInertSoilOrganicC((capnp::uint)so->vo_InertSoilOrganicC.size()));
  ...
  setCapnpList(
      so->vo_NetNMineralisationRate,
      builder.initNetNMineralisationRate((capnp::uint)so->vo_NetNMineralisationRate.size()));
  ...
  setCapnpList(so->vo_SMB_CO2EvolutionRate,
               builder.initSmbCO2EvolutionRate((capnp::uint)so->vo_SMB_CO2EvolutionRate.size()));
  setCapnpList(so->vo_SMB_FastDelta,
               builder.initSmbFastDelta((capnp::uint)so->vo_SMB_FastDelta.size()));
  setCapnpList(so->vo_SMB_SlowDelta,
               builder.initSmbSlowDelta((capnp::uint)so->vo_SMB_SlowDelta.size()));
  setCapnpList(so->vs_SoilMineralNContent,
               builder.initVsSoilMineralNContent((capnp::uint)so->vs_SoilMineralNContent.size()));
  setCapnpList(so->vo_SoilOrganicC,
               builder.initSoilOrganicC((capnp::uint)so->vo_SoilOrganicC.size()));
  setCapnpList(so->vo_SOM_FastDelta,
               builder.initSomFastDelta((capnp::uint)so->vo_SOM_FastDelta.size()));
  setCapnpList(so->vo_SOM_FastInput,
               builder.initSomFastInput((capnp::uint)so->vo_SOM_FastInput.size()));
  setCapnpList(so->vo_SOM_SlowDelta,
...
void addOrganicMatter(SoilOrganic *so, const OrganicMatterParameters &params, double amount,
                      double nConcentration, size_t intoLayerIndex) {
...
kj::Own<SoilOrganic>
makeSoilOrganic(SoilColumn &soilColumn,
                mas::schema::model::monica::SoilOrganicModuleState::Reader reader,
