...
double stics::vnit(const monica::SticsParameters &ps, double NH4, double pH, double soilT,
  ...
  switch (ps.code_tnit) {
  case 1:
    fTres = nit::fTstep(soilT, ps.tnitmin, ps.tnitopt, ps.tnitop2, ps.tnitmax);
    break;
  case 2:
    fTres = nit::fTgauss(soilT, ps.tnitopt_gauss, ps.scale_tnitopt);
    break;
  default:;
  }

  return vnitpot * fNH4res * nit::fpH(ps.pHminnit, pH, ps.pHmaxnit) * fTres *
         nit::fWFPS(wfps, ps.hminn, ps.hoptn, fc, sat);
}

namespace denit {

double fNO3(double NO3, double w, double Kd) { return NO3 / (NO3 + (w * Kd)); }
...
double fNO3(double NO3) { return NO3 / (NO3 + 1); }
