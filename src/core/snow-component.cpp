...
void monica::snowcomponent::initialize(SnowComponent *sc, SoilColumn *soilColumn,
...
void monica::snowcomponent::deserialize(
    SnowComponent *sc, mas::schema::model::monica::SnowModuleState::Reader reader) {
...
void monica::snowcomponent::serialize(
    const SnowComponent *sc, mas::schema::model::monica::SnowModuleState::Builder builder) {
 ...
 */
void monica::snowcomponent::calcSnowLayer(SnowComponent *sc, double mean_air_temperature,
                                          double net_precipitation) {
  // Calcs netto precipitation
  double net_precipitation_snow = 0.0;
  double net_precipitation_water = 0.0;
  net_precipitation = calcNetPrecipitation(sc, mean_air_temperature, net_precipitation,
  ...
  double vm_SnowWaterEquivalent =
      sc->vm_FrozenWaterInSnow + sc->vm_LiquidWaterInSnow; // snow water equivalent [mm]
 ...
 * @return
 */
double monica::snowcomponent::calcNetPrecipitation(const SnowComponent *sc,
                                                   ...
                                                   double &net_precipitation_snow)  // return values
{
  ...
  net_precipitation_snow =
      (1.0 - liquid_water_precipitation) * sc->vm_CorrectionSnow * net_precipitation;
  ...
  if (refreeze_helper < sc->vm_RefreezeTemperature) {
    if (sc->vm_LiquidWaterInSnow > 0.0) {
      refreeze = sc->vm_RefreezeP1 *
                 pow((sc->vm_RefreezeTemperature - refreeze_helper), sc->vm_RefreezeP2);
    }
...
double monica::snowcomponent::calcNewSnowDensity(const SnowComponent *sc,
  ...
  if (net_precipitation_snow <= 0.0) {
    ...
    new_snow_density =
        sc->vm_NewSnowDensityMin + sc->vm_SnowMaxAdditionalDensity * snow_density_factor;
  }
 ...
 * @return
 */
double monica::snowcomponent::calcPotentialInfiltration(SnowComponent *sc, double net_precipitation,
...
void monica::snowcomponent::calcSnowDepth(SnowComponent *sc, double snow_water_equivalent) {
  ...
  if (snow_water_equivalent <= 0.0) {
    ...
    sc->vm_SnowDepth =
        snow_water_equivalent * pm_WaterDensity / sc->vm_SnowDensity; // [mm * kg dm-3 kg-1 dm3]
