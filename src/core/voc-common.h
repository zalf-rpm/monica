...
//! light modifier, Guenther et al. 1993; (Staudt et al. 2004 f. Q.ilex: 0.0041,
//! Bertin et al. 1997 f. Q.ilex: 0.00147, Harley et al. 2004 f. M.indica:
//! 0.00145)
static double const ALPHA{0.0027};
//! monoterpene scaling factor, Guenther et al. 1995 (cit. in Guenther 1999 says
//! this value originates from Guenther 1993)
static double const BETA{0.09};
//!  fraction of electrons used from excess electron transport (−), from Sun et
//!  al. 2012 dataset, Grote et al. 2014
static double const C1{0.17650}; // fw: 0.17650e-3 in Grote et al. 2014 to
                                 // preserve the constant relation to C2;
//! fraction of electrons used from photosynthetic electron transport (−),from
//! Sun et al. 2012 dataset, Grote et al. 2014
static double const C2{0.00280}; // fw: 0.00280e-3 (umol m-2 s-1) in Grote et
                                 // al. 2014; here in nmol m-2 s-1;
//! emission-class dependent empirical coefficient for temperature acitivity
//! factor of isoprene from MEGAN v2.1 (Guenther et al. 2012)
static double const CEO_ISO{2.0};
//! emission-class dependent empirical coefficient for temperature acitivity
//! factor of a-pinene, b-pinene, oMT, Limonen, etc from MEGAN v2.1 (Guenther et
//! al. 2012)
static double const CEO_MONO{1.83};
//! first temperature modifier (J mol-1), Guenther et al. 1993; (Staudt et al.
//! 2004 f. Q.ilex: 87620, Bertin et al. 1997 f. Q.ilex: 74050, Harley et al. f.
//! M.indica: 124600, in WIMOVAC 95100)
static double const CT1{95000.0};
//! second temperature modifier (J mol-1), Guenther et al. 1993; (Staudt et al.
//! 2004 f. Q.ilex: 188200, Bertin et al. 1997 f. Q.ilex: 638600, Harley et al.
//! f. M.indica: 254800)
static double const CT2{230000.0};
//! radiation modifier, Guenther et al. 1993; (Staudt et al. 2004 f.
//! Q.ilex: 1.04, Bertin et al. 1997 f. Q.ilex: 1.21, Harley et al. 2004 f.
//! M.indica: 1.218)
static double const CL1{1.066};
//! saturating amount of electrons that can be supplied from other sources (μmol
//! m−2 s−1), Sun et al. 2012 dataset, Grote et al. 2014 (delta J_sat in paper)
static double const GAMMA_MAX{34.0};
//! reference photosynthetically active quantum flux density/light density
//! (standard conditions) (umol m-2 s-1), Guenther et al. 1993
...
//! temperature with maximum emission (K), Guenther et al. 1993; (Staudt et al.
//! 2004 f. Q.ilex: 317, Bertin et al. 1997 f. Q.ilex: 311.6, Harley et al. f.
//! M.indica: 313.4, in WIMOVAC 311.83)
...
struct SpeciesData {
  ...
  // common
  double EF_MONOS{0.0}; //!< emission rate of stored terpenes under standard
  ...
  double HDJ{220000.0}; //! fw: "HDJ": curvature parameter of jMax (J mol-1)
                        ...
                        //! 220000; Harley et al. 1992: 201000)
  ...
  // vegstructure  specific_foliage_area sla_vtfl  double  V : F  0.0  m ^ 2 : g
  // : 10 ^ -3 specific leaf area (pc_SpecificLeafArea /
  // cps.cultivarParams.pc_SpecificLeafArea)
...
struct MicroClimateData {
  ...
  //  microclimate  ts_sunlitfoliagefraction_fl ts_sunlitfoliagefraction_fl
  //  double  F  0.0 ?
  double sunlitfoliagefraction{0};
  // fraction of sunlit foliage over the past 24 hours per canopy layer
  // microclimate  sunlitfoliagefraction24_fl  sunlitfoliagefraction24_fl double
  // F  0.0 ?
...
struct Emissions {
  void serialize(mas::schema::model::monica::Voc::Emissions::Builder builder) const {
    ...
    {
      auto monos = builder.initSpeciesIdToMonoterpeneEmission(
          (capnp::uint)speciesId_2_monoterpene_emission.size());
      capnp::uint i = 0;
  ...
  void deserialize(mas::schema::model::monica::Voc::Emissions::Reader reader) {
    ...
    for (auto e : reader.getSpeciesIdToIsopreneEmission())
      speciesId_2_isoprene_emission[(int)e.getSpeciesId()] = e.getEmission();

    speciesId_2_monoterpene_emission.clear();
    for (auto e : reader.getSpeciesIdToMonoterpeneEmission())
      speciesId_2_monoterpene_emission[(int)e.getSpeciesId()] = e.getEmission();
