...
void deserialize(
  ...
  soillayer::deserialize(&st->soilColumnGroundLayer,
                         reader.getSoilColumnVtGroundLayer());
  soillayer::deserialize(&st->soilColumnBottomLayer,
                         reader.getSoilColumnVtBottomLayer());
  soiltemperaturemoduleparameters::deserialize(&st->params,
  ...
  setFromCapnpList(st->matrixPrimaryDiagonal,
                   reader.getMatrixPrimaryDiagonal());
  setFromCapnpList(st->matrixSecondaryDiagonal,
                   reader.getMatrixSecundaryDiagonal());
...
void serialize(
  ...
  soillayer::serialize(&st->soilColumnGroundLayer,
                       builder.initSoilColumnVtGroundLayer());
  soillayer::serialize(&st->soilColumnBottomLayer,
                       builder.initSoilColumnVtBottomLayer());
  soiltemperaturemoduleparameters::serialize(&st->params,
  ...
  setCapnpList(
      st->soilTemperature,
      builder.initSoilTemperature((capnp::uint)st->soilTemperature.size()));
  setCapnpList(st->V, builder.initV((capnp::uint)st->V.size()));
  setCapnpList(st->volumeMatrix,
               builder.initVolumeMatrix((capnp::uint)st->volumeMatrix.size()));
  setCapnpList(
      st->volumeMatrixOld,
      builder.initVolumeMatrixOld((capnp::uint)st->volumeMatrixOld.size()));
  setCapnpList(st->B, builder.initB((capnp::uint)st->B.size()));
  setCapnpList(st->matrixPrimaryDiagonal,
               builder.initMatrixPrimaryDiagonal(
                   (capnp::uint)st->matrixPrimaryDiagonal.size()));
  setCapnpList(st->matrixSecondaryDiagonal,
               builder.initMatrixSecundaryDiagonal(
                   (capnp::uint)st->matrixSecondaryDiagonal.size()));
  setCapnpList(
      st->heatConductivity,
      builder.initHeatConductivity((capnp::uint)st->heatConductivity.size()));
  setCapnpList(st->heatConductivityMean,
               builder.initHeatConductivityMean(
                   (capnp::uint)st->heatConductivityMean.size()));
  setCapnpList(st->heatCapacity,
               builder.initHeatCapacity((capnp::uint)st->heatCapacity.size()));
...
void step(SoilTemperature *st, double tmin, double tmax, double globrad) {
  ...
  st->soilSurfaceTemperature = calcSoilSurfaceTemperature(
      st, st->soilSurfaceTemperature, tmin, tmax, globrad);
  st->soilColumn->vt_SoilSurfaceTemperature = st->soilSurfaceTemperature;
  st->heatFlow[0] =
      st->soilSurfaceTemperature * st->B[0] * st->heatConductivityMean[0];

  for (size_t i = 0; i < st->noOfTempLayers; i++) {
    st->solution[i] = (st->volumeMatrixOld[i] +
  ...
  for (size_t i = 1; i < st->noOfTempLayers; i++) {
    st->matrixLowerTriangle[i] =
        st->matrixSecondaryDiagonal[i] / st->matrixDiagonal[i - 1];
    st->matrixDiagonal[i] =
  ...
  for (size_t i = 1; i < st->noOfTempLayers; i++) {
    st->solution[i] =
        st->solution[i] - (st->matrixLowerTriangle[i] * st->solution[i - 1]);
  }

  st->solution[bottomLayer] =
      st->solution[bottomLayer] / st->matrixDiagonal[bottomLayer];
...
double calcSoilSurfaceTemperature(const SoilTemperature *st,
  ...
  const double soilCoverage =
      st->monica->currentCropModule
          ? st->monica->currentCropModule->vc_SoilCoverage
          : 0.0;
  const double shadingCoefficient =
      0.1 + ((soilCoverage * st->dampingFactor) +
             ((1 - soilCoverage) * (1 - st->dampingFactor)));
  ...
  if (st->monica->soilMoisture->snowComponent->vm_SnowDepth > 0.0) {
    soilSurfaceTemperature =
        st->monica->soilMoisture->frostComponent->vm_TemperatureUnderSnow;
  }
...
makeSoilTemperature(MonicaModel &mm,
  ...
  for (size_t i = 0; i < soilNols; i++) {
    st->soilTemperature[i] =
        ((1.0 - (double(i) / soilNols)) * initialSurfaceTemp) +
  ...
  st->soilTemperature[groundLayer] =
      (st->soilTemperature[groundLayer - 1] + baseTemp) * 0.5;
  ...
  for (size_t i = 1; i < st->noOfTempLayers; i++) {
    const double lti_1 =
        soilTemperatureLayerAt(st.get(), i - 1).vs_LayerThickness;
  ...
  for (size_t i = 0; i < st->noOfSoilLayers; i++) {
    const double sbdi =
        soilTemperatureLayerAt(st.get(), i)._sps.vs_SoilBulkDensity();
    ...
    const double somi =
        soilTemperatureLayerAt(st.get(), i)._sps.vs_SoilOrganicMatter() / da *
        sbdi;
  ...
  for (size_t i = 1; i < st->noOfTempLayers; i++) {
    ...
    st->heatConductivityMean[i] =
        ((lti_1 * hci_1) + (lti * hci)) / (lti + lti_1);
  }
  ...
  for (size_t i = 0; i < st->noOfTempLayers; i++) {
    st->matrixPrimaryDiagonal[i] = st->volumeMatrix[i] -
                                   st->matrixSecondaryDiagonal[i] -
                                   st->matrixSecondaryDiagonal[i + 1];
  }
...
kj::Own<SoilTemperature> makeSoilTemperature(
  ...
  st->matrixLowerTriangle.resize(st->noOfTempLayers);
  st->heatFlow.assign(st->noOfTempLayers, 0.0);
  soiltemperature::deserialize(st.get(), reader);
  return st;
}
