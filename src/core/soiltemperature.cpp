...
void serialize(const SoilTemperature *st,
  ...
  setCapnpList(st->soilTemperature,
  ...
  setCapnpList(st->volumeMatrixOld,
  ...
  setCapnpList(st->matrixPrimaryDiagonal,
               builder.initMatrixPrimaryDiagonal((capnp::uint)st->matrixPrimaryDiagonal.size()));
  setCapnpList(st->matrixSecondaryDiagonal, builder.initMatrixSecundaryDiagonal(
                                                (capnp::uint)st->matrixSecondaryDiagonal.size()));
  setCapnpList(st->heatConductivity,
               builder.initHeatConductivity((capnp::uint)st->heatConductivity.size()));
  setCapnpList(st->heatConductivityMean,
               builder.initHeatConductivityMean((capnp::uint)st->heatConductivityMean.size()));
...
void step(SoilTemperature *st, double tmin, double tmax, double globrad) {
  ...
  for (size_t i = 0; i < st->noOfTempLayers; i++) {
    st->solution[i] =
        (st->volumeMatrixOld[i] + (st->volumeMatrix[i] - st->volumeMatrixOld[i]) /
...
double calcSoilSurfaceTemperature(const SoilTemperature *st, double prevDaySoilSurfaceTemperature,
                                  double tmin, double tmax, double globrad) {
  globrad = max(8.33, globrad);

  const double soilCoverage =
      st->monica->currentCropModule ? st->monica->currentCropModule->vc_SoilCoverage : 0.0;
  ...
  double soilSurfaceTemperature =
      (1.0 - shadingCoefficient) * (tmin + ((tmax - tmin) * pow((0.03 * globrad), 0.5))) +
...
kj::Own<SoilTemperature> makeSoilTemperature(MonicaModel &mm,
  ...
  if (!st->soilColumn->layers.empty())
    st->soilColumnGroundLayer = st->soilColumnBottomLayer = st->soilColumn->layers.back();
  ...
  for (size_t i = 0; i < soilNols; i++) {
    st->soilTemperature[i] =
        ((1.0 - (double(i) / soilNols)) * initialSurfaceTemp) + ((double(i) / soilNols) * baseTemp);
  }
  ...
  for (size_t i = 0; i < st->noOfSoilLayers; i++) {
    const double sbdi = soillayer::soilBulkDensity(&soilTemperatureLayerAt(st.get(), i));
    const double smi = soilMoistureConst;
    st->heatConductivity[i] =
        ((3.0 * (sbdi / 1000.0) - 1.7) * 0.001) /
        (1.0 + (11.5 - 5.0 * (sbdi / 1000.0)) * exp((-50.0) * pow((smi / (sbdi / 1000.0)), 1.5))) *
        86400.0 * ts * 100.0 * 4.184;

    const double sati = soilTemperatureLayerAt(st.get(), i).vs_Saturation;
    const double somi =
        soillayer::soilOrganicMatter(&soilTemperatureLayerAt(st.get(), i)) / da * sbdi;
    st->heatCapacity[i] = (smi * dw * cw) + ((sati - smi) * da * ca) + (somi * dh * ch) +
                          ((1.0 - sati - somi) * dq * cq);
  }
