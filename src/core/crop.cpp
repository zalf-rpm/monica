...
Crop::Crop(const Crop &other)
    : _isValid(other._isValid), _speciesName(other._speciesName),
      _cultivarName(other._cultivarName), _seedDate(other._seedDate),
      _harvestDate(other._harvestDate), _isWinterCrop(other._isWinterCrop),
      _isPerennialCrop(other._isPerennialCrop),
      _cuttingDates(other._cuttingDates), _cropParams(other._cropParams),
      _separatePerennialCropParams(
          other._separatePerennialCropParams
              ? kj::heap<CropParameters>(other._perennialCropParams)
              : kj::Own<CropParameters>()),
      _perennialCropParams(other._separatePerennialCropParams
                               ? *_separatePerennialCropParams.get()
                               : _cropParams),
...
Crop::Crop(json11::Json j) : _perennialCropParams(_cropParams) { merge(j); }
...
void Crop::deserialize(mas::schema::model::monica::CropState::Reader reader) {
  ...
  if (reader.hasIsWinterCrop())
    _isWinterCrop.setValue(reader.getIsWinterCrop().getValue());
  if (reader.hasIsPerennialCrop())
    _isPerennialCrop.setValue(reader.getIsPerennialCrop().getValue());
  setFromComplexCapnpList(_cuttingDates, reader.getCuttingDates());
  if (!reader.hasCropParams())
    _isValid = false;
  else
    _cropParams.deserialize(reader.getCropParams());
  if (reader.hasPerennialCropParams()) {
    _separatePerennialCropParams = nullptr;
    _separatePerennialCropParams =
        kj::heap<CropParameters>(reader.getPerennialCropParams());
...
void Crop::serialize(
    mas::schema::model::monica::CropState::Builder builder) const {
  ...
  if (_isWinterCrop.isValue())
    builder.initIsWinterCrop().setValue(_isWinterCrop.value());
  if (_isPerennialCrop.isValue())
    builder.initIsPerennialCrop().setValue(_isPerennialCrop.value());
  setComplexCapnpList(_cuttingDates, builder.initCuttingDates(
                                         (capnp::uint)_cuttingDates.size()));
  if (isValid())
    _cropParams.serialize(builder.initCropParams());
  if (_separatePerennialCropParams)
    _separatePerennialCropParams->serialize(builder.initPerennialCropParams());
