...
double leaf_senescence_reduction_Ac(double fO3l, double reldev,
  ...
  if (reldev > crit_reldev) {
    fLS = std::max((1.0 - senescence_impact_max),
                   1.0 - senescence_impact_max * (reldev - crit_reldev) /
                             (fO3l - crit_reldev)); // correction of rate due to
                                                    // O3 cumulative uptake
  }
  return fLS;
}
...
O3_impact_out O3impact::O3_impact_hourly(O3_impact_in in, O3_impact_params par,
                                         bool WaterDeficitResponseStomata) {
  O3_impact_out out;
