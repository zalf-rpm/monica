...
soil_transport_step :: proc(st: ^Soil_Transport) {
	...
	for i_TimeStep := 0; f64(i_TimeStep) < (1.0 / minTimeStepFactor); i_TimeStep += 1 {
		n_transport(st, st.env_params.p_LeachingDepth, minTimeStepFactor)
	}

	for i in 0 ..< nols {
		st.soil_no3[i] = st.soil_no3_aq[i] * st.soil_column.layers[i].soil_moisture_m3

		if st.soil_no3[i] < 0.0 {
			st.soil_no3[i] = 0.0
		}

		st.soil_column.layers[i].soil_no3 = st.soil_no3[i]
	}
}
...
n_uptake :: proc(st: ^Soil_Transport) {
	...
	for i in 0 ..< nols {
		...
		if st.vc_NUptakeFromLayer[i] >
		   ((st.soil_no3[i] * lti) - st.crop_mod_params.pc_MinimumAvailableN) {
			st.vc_NUptakeFromLayer[i] =
				((st.soil_no3[i] * lti) - st.crop_mod_params.pc_MinimumAvailableN)
...
n_transport :: proc(st: ^Soil_Transport, leachingDepth, timeStepFactor: f64) {
	diffusionCoeffStandard := st.mod_params.pq_DiffusionCoefficientStandard // [m2 d-1]; old D0
	AD := st.mod_params.pq_AD // Factor a in Kersebaum 1989 p.24 for Loess soils
	dispersionLength := st.mod_params.pq_DispersionLength // [m]
	soilProfile := 0.0
		...
		if i == 0 {
			st.dispersion_coeff[i] =
				soilMoistureGradient[i] *
					(st.diffusion_coeff[i] + dispersionLength * st.pore_water_velocity[i]) -
				(0.5 * lti * libc.fabs(pri)) +
				((0.5 * st.env_params.p_timeStep * timeStepFactor * libc.fabs((pri + pr0) / 2.0)) *
	...
	if st.percolation_rate[leachingDepthLayerIndex] > 0.0 {
		...
		if leachingDepthLayerIndex < nols - 1 {
			...
			st.leaching_at_boundary +=
				...
				((st.dispersion_coeff[leachingDepthLayerIndex] * (NO3 - NO3_u)) /
						(lt * lt) *
						10000.0 *
						lt) // [kg ha-1]
		} else {
		...
		if leachingDepthLayerIndex < nols - 1 {
			...
			st.leaching_at_boundary +=
				...
				st.dispersion_coeff[leachingDepthLayerIndex] *
					(NO3 - NO3_u) /
