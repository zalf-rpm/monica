#!/usr/bin/env bash
# Phase 6 checkpoint 1 differential test: MonicaModel scaffolding + standalone
# helpers (see monica_model_ref_main.cpp's header comment).
#
# Usage:  bash odin/tests/cpp_ref/run_monica_model.sh
#
# Requires a configured AND BUILT CMake tree, same as run_crop_module_step.sh.
set -uo pipefail
...
mkdir -p build/ref build/ref/monica_model
...
cat > build/ref/_build_monica_model.bat <<EOF
@echo off
call "$VCVARS" >nul
cd /d $(cygpath -w "$REPO")
cl /EHsc /std:c++17 /nologo /Zi /D_USE_MATH_DEFINES $FLAGS /I odin\tests\cpp_ref /Fe:build\ref\monica_model_ref.exe /Fo:"build\ref\monica_model\\\\" /Fd:"build\ref\monica_model\\\\" odin\tests\cpp_ref\monica_model_ref_main.cpp /link /LIBPATH:build /LIBPATH:$VCPKG_LIB monica_run_lib.lib monica_lib.lib climate\climate-file-io\climate_file_io_lib.lib climate\climate-file-io\climate_common\climate_common_lib.lib common\common_lib.lib common\json11\json11_lib.lib common\json11\date\date_lib.lib common\tools\helpers\helpers_lib.lib tools\debug\debug_lib.lib soil\soil_lib.lib mas-infrastructure\capnproto_schemas\capnp_schemas_lib.lib capnp-json.lib capnp-rpc.lib capnp.lib capnpc.lib kj-http.lib kj-gzip.lib kj-async.lib kj.lib zlibd.lib libsodium.lib advapi32.lib ws2_32.lib
EOF
cmd //c "$(cygpath -w build/ref/_build_monica_model.bat)" > build/ref/_build_monica_model.log 2>&1
if [ ! -f build/ref/monica_model_ref.exe ]; then
  echo "FAILED to build the C++ driver; see build/ref/_build_monica_model.log"
  grep -iE "error" build/ref/_build_monica_model.log | head -20
  exit 1
fi

echo "== building Odin driver"
"$ODIN" build odin/tests/monica_model_ref -out:build/ref/monica_model_ref_odin.exe || exit 1

echo "== running both on $SIM"
./build/ref/monica_model_ref.exe      "$SIM" 2>build/ref/monica_model_cpp.err  | tr -d '\r' | sort > build/ref/monica_model_cpp.txt
./build/ref/monica_model_ref_odin.exe "$SIM" 2>build/ref/monica_model_odin.err | tr -d '\r' | sort > build/ref/monica_model_odin.txt

n=$(wc -l < build/ref/monica_model_cpp.txt)
if [ "$n" -lt 50 ]; then
  echo "FAIL - C++ driver produced only $n trace lines; stderr:"
  head -20 build/ref/monica_model_cpp.err
  exit 1
fi

echo "== diffing"
if diff build/ref/monica_model_cpp.txt build/ref/monica_model_odin.txt > build/ref/monica_model_diff.txt; then
  echo "PASS - MonicaModel scaffolding + standalone helpers trace identical ($n lines)"
  exit 0
else
  echo "FAIL - first differing fields:"
  head -40 build/ref/monica_model_diff.txt
  exit 1
fi
