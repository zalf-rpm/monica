#!/usr/bin/env bash
# Phase 6 checkpoint 2 prerequisite differential test: the cropmodule:: yield/
# N-content getters and applyCutting (see crop_module_yield_ref_main.cpp's
# header comment).
#
# Usage:  bash odin/tests/cpp_ref/run_crop_module_yield.sh
set -uo pipefail
...
mkdir -p build/ref build/ref/crop_module_yield
...
cat > build/ref/_build_crop_module_yield.bat <<EOF
@echo off
call "$VCVARS" >nul
cd /d $(cygpath -w "$REPO")
cl /EHsc /std:c++17 /nologo /Zi /D_USE_MATH_DEFINES $FLAGS /I odin\tests\cpp_ref /Fe:build\ref\crop_module_yield_ref.exe /Fo:"build\ref\crop_module_yield\\\\" /Fd:"build\ref\crop_module_yield\\\\" odin\tests\cpp_ref\crop_module_yield_ref_main.cpp /link /LIBPATH:build /LIBPATH:$VCPKG_LIB monica_run_lib.lib monica_lib.lib climate\climate-file-io\climate_file_io_lib.lib climate\climate-file-io\climate_common\climate_common_lib.lib common\common_lib.lib common\json11\json11_lib.lib common\json11\date\date_lib.lib common\tools\helpers\helpers_lib.lib tools\debug\debug_lib.lib soil\soil_lib.lib mas-infrastructure\capnproto_schemas\capnp_schemas_lib.lib capnp-json.lib capnp-rpc.lib capnp.lib capnpc.lib kj-http.lib kj-gzip.lib kj-async.lib kj.lib zlibd.lib libsodium.lib advapi32.lib ws2_32.lib
EOF
cmd //c "$(cygpath -w build/ref/_build_crop_module_yield.bat)" > build/ref/_build_crop_module_yield.log 2>&1
if [ ! -f build/ref/crop_module_yield_ref.exe ]; then
  echo "FAILED to build the C++ driver; see build/ref/_build_crop_module_yield.log"
  grep -iE "error" build/ref/_build_crop_module_yield.log | head -20
  exit 1
fi

echo "== building Odin driver"
"$ODIN" build odin/tests/crop_module_yield_ref -out:build/ref/crop_module_yield_ref_odin.exe || exit 1

echo "== running both on $SIM / $CLIMATE ($NUM_DAYS days)"
./build/ref/crop_module_yield_ref.exe      "$SIM" "$CLIMATE" "$NUM_DAYS" 2>build/ref/crop_module_yield_cpp.err  | tr -d '\r' | sort > build/ref/crop_module_yield_cpp.txt
./build/ref/crop_module_yield_ref_odin.exe "$SIM" "$CLIMATE" "$NUM_DAYS" 2>build/ref/crop_module_yield_odin.err | tr -d '\r' | sort > build/ref/crop_module_yield_odin.txt

n=$(wc -l < build/ref/crop_module_yield_cpp.txt)
if [ "$n" -lt 30 ]; then
  echo "FAIL - C++ driver produced only $n trace lines; stderr:"
  head -20 build/ref/crop_module_yield_cpp.err
  exit 1
fi

echo "== diffing"
if diff build/ref/crop_module_yield_cpp.txt build/ref/crop_module_yield_odin.txt > build/ref/crop_module_yield_diff.txt; then
  echo "PASS - yield getters + applyCutting trace identical ($n lines)"
  exit 0
else
  echo "FAIL - first differing fields:"
  head -40 build/ref/crop_module_yield_diff.txt
  exit 1
fi
