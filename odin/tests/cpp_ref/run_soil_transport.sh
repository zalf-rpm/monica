#!/usr/bin/env bash
# Phase 4 differential test: soiltransport.cpp.
#
# Builds a real MonicaModel from sim-min.json via makeMonicaModel, chains
# soilmoisture::step (already proven identical) before soiltransport::step
# each day over NUM_DAYS days of the real Hohenfinow2 climate-min.csv record,
# with a synthetic deterministic groundwater-depth sequence and bare soil (no
# live crop) - see soil_transport_ref_main.cpp's header comment.
#
# Usage:  bash odin/tests/cpp_ref/run_soil_transport.sh
#
...
mkdir -p build/ref build/ref/soil_transport
...
cat > build/ref/_build_soil_transport.bat <<EOF
@echo off
call "$VCVARS" >nul
cd /d $(cygpath -w "$REPO")
cl /EHsc /std:c++17 /nologo /Zi /D_USE_MATH_DEFINES $FLAGS /I odin\tests\cpp_ref /Fe:build\ref\soil_transport_ref.exe /Fo:"build\ref\soil_transport\\\\" /Fd:"build\ref\soil_transport\\\\" odin\tests\cpp_ref\soil_transport_ref_main.cpp /link /LIBPATH:build /LIBPATH:$VCPKG_LIB monica_run_lib.lib monica_lib.lib climate\climate-file-io\climate_file_io_lib.lib climate\climate-file-io\climate_common\climate_common_lib.lib common\common_lib.lib common\json11\json11_lib.lib common\json11\date\date_lib.lib common\tools\helpers\helpers_lib.lib tools\debug\debug_lib.lib soil\soil_lib.lib mas-infrastructure\capnproto_schemas\capnp_schemas_lib.lib capnp-json.lib capnp-rpc.lib capnp.lib capnpc.lib kj-http.lib kj-gzip.lib kj-async.lib kj.lib zlibd.lib libsodium.lib advapi32.lib ws2_32.lib
EOF
cmd //c "$(cygpath -w build/ref/_build_soil_transport.bat)" > build/ref/_build_soil_transport.log 2>&1
if [ ! -f build/ref/soil_transport_ref.exe ]; then
  echo "FAILED to build the C++ driver; see build/ref/_build_soil_transport.log"
  grep -iE "error" build/ref/_build_soil_transport.log | head -12
  exit 1
fi

echo "== building Odin driver"
"$ODIN" build odin/tests/soil_transport_ref -out:build/ref/soil_transport_ref_odin.exe || exit 1

echo "== running both on $SIM / $CLIMATE ($NUM_DAYS days)"
./build/ref/soil_transport_ref.exe      "$SIM" "$CLIMATE" "$NUM_DAYS" 2>build/ref/soil_transport_cpp.err  | tr -d '\r' | sort > build/ref/soil_transport_cpp.txt
./build/ref/soil_transport_ref_odin.exe "$SIM" "$CLIMATE" "$NUM_DAYS" 2>build/ref/soil_transport_odin.err | tr -d '\r' | sort > build/ref/soil_transport_odin.txt

n=$(wc -l < build/ref/soil_transport_cpp.txt)
if [ "$n" -lt 1000 ]; then
  echo "FAIL - C++ driver produced only $n trace lines; stderr:"
  head -10 build/ref/soil_transport_cpp.err
  exit 1
fi

echo "== diffing"
if diff build/ref/soil_transport_cpp.txt build/ref/soil_transport_odin.txt > build/ref/soil_transport_diff.txt; then
  echo "PASS - soiltransport daily trace identical over $NUM_DAYS days ($n lines)"
  exit 0
else
  echo "FAIL - first differing fields:"
  head -20 build/ref/soil_transport_diff.txt
  exit 1
fi
