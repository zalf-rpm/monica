#!/usr/bin/env bash
# Phase 6 checkpoint 5 differential test: cultivation_method.odin (see
# cultivation_method_ref_main.cpp's header comment).
#
# Usage:  bash odin/tests/cpp_ref/run_cultivation_method.sh
set -uo pipefail
...
mkdir -p build/ref build/ref/cultivation_method
...
cat > build/ref/_build_cultivation_method.bat <<EOF
@echo off
call "$VCVARS" >nul
cd /d $(cygpath -w "$REPO")
cl /EHsc /std:c++17 /nologo /Zi /D_USE_MATH_DEFINES $FLAGS /I odin\tests\cpp_ref /Fe:build\ref\cultivation_method_ref.exe /Fo:"build\ref\cultivation_method\\\\" /Fd:"build\ref\cultivation_method\\\\" odin\tests\cpp_ref\cultivation_method_ref_main.cpp /link /LIBPATH:build /LIBPATH:$VCPKG_LIB monica_run_lib.lib monica_lib.lib climate\climate-file-io\climate_file_io_lib.lib climate\climate-file-io\climate_common\climate_common_lib.lib common\common_lib.lib common\json11\json11_lib.lib common\json11\date\date_lib.lib common\tools\helpers\helpers_lib.lib tools\debug\debug_lib.lib soil\soil_lib.lib mas-infrastructure\capnproto_schemas\capnp_schemas_lib.lib capnp-json.lib capnp-rpc.lib capnp.lib capnpc.lib kj-http.lib kj-gzip.lib kj-async.lib kj.lib zlibd.lib libsodium.lib advapi32.lib ws2_32.lib
EOF
cmd //c "$(cygpath -w build/ref/_build_cultivation_method.bat)" > build/ref/_build_cultivation_method.log 2>&1
if [ ! -f build/ref/cultivation_method_ref.exe ]; then
  echo "FAILED to build the C++ driver; see build/ref/_build_cultivation_method.log"
  grep -iE "error" build/ref/_build_cultivation_method.log | head -30
  exit 1
fi

echo "== building Odin driver"
"$ODIN" build odin/tests/cultivation_method_ref -out:build/ref/cultivation_method_ref_odin.exe || exit 1

echo "== running both on $SIM / $CLIMATE ($NUM_DAYS days)"
./build/ref/cultivation_method_ref.exe      "$SIM" "$CLIMATE" "$NUM_DAYS" 2>build/ref/cultivation_method_cpp.err  | tr -d '\r' | sort > build/ref/cultivation_method_cpp.txt
./build/ref/cultivation_method_ref_odin.exe "$SIM" "$CLIMATE" "$NUM_DAYS" 2>build/ref/cultivation_method_odin.err | tr -d '\r' | sort > build/ref/cultivation_method_odin.txt

n=$(wc -l < build/ref/cultivation_method_cpp.txt)
if [ "$n" -lt 30 ]; then
  echo "FAIL - C++ driver produced only $n trace lines; stderr:"
  head -30 build/ref/cultivation_method_cpp.err
  exit 1
fi

echo "== diffing"
if diff build/ref/cultivation_method_cpp.txt build/ref/cultivation_method_odin.txt > build/ref/cultivation_method_diff.txt; then
  echo "PASS - cultivation_method.odin trace identical ($n lines)"
  exit 0
else
  echo "FAIL - first differing fields:"
  head -60 build/ref/cultivation_method_diff.txt
  exit 1
fi
