// Odin side of the phase 4 soiltransport differential test.
//
// Must emit byte-identical output to
// odin/tests/cpp_ref/soil_transport_ref_main.cpp - see that file's header
// comment for why this driver chains soil_moisture_step before
// soil_transport_step each day.
// Run odin/tests/cpp_ref/run_soil_transport.sh to build both and diff them.
package soil_transport_ref
...
main :: proc() {
	args := os.args
	if len(args) < 4 {
		fmt.eprintln("usage: soil_transport_ref <pathToSimJson> <pathToClimateCsv> <numDays>")
		os.exit(2)
	}
	...
	// --- build SoilColumn, SoilMoisture, SoilTransport - mirroring
	...
	for day in 0 ..< n {
		...
		core.soil_transport_step(&st)

		tr.set_day(&t, day)
		tr.dump(&t, "soilTransport", st)
