...
def main():
  ...
  if (rank == 0):
    # only one processor reads in the meta information
		splittedGridDataMap = splitAsciiGrid("/media/san1_data1/data1/berg/carbiocial/cluster-code/carbiocial-cluster-code/CarbiocialRasterSoils.asc", size)
  ...
  for coord, profileId in nodeSpecificDataMap.iteritems():
    ...
    print rank, "###################################"
    print rank, " ", index, "/", len(nodeSpecificDataMap), " coord: ", coord, " profileId: ", monica_simulation_config.getProfileId()
    ...
    year2yield = {}
    for y in range(50):
      year2yield[y] = 5.434343434343434343434343434;
    ...
    if len(year2yield) > 0:
      ...
      _coord2year2yield[(row, col)] = y2y

      # remove simulation result dir
      #if remove_monica_files_after_simulation:
      #  shutil.rmtree(path, True)

    #coord2year2yield += ",".join(map(str, _coord2year2yield.iteritems()))
    coord2year2yield += len(_coord2year2yield)
    #print rank, "###################################"
  ...
  print "rank: ", rank, " ", coord2year2yield

  if rank == 0:

    print "outputing results ..."

    print coord2year2yield
